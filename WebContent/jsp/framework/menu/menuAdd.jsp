...
<BZ:head>
<title>菜单添加页面</title>
<BZ:script tree="true" isEdit="true" isAjax="true"/>
<script type="text/javascript" language="javascript">
	...
	function tijiao()
	{
		...
		document.srcForm.action=path+"menu/menuAdd.action";
			...
			if(moduleId!="") {
					for(var i=0;i<resArr.length;i++) {
							if(resArr[i].moduleId==moduleId) {
								res.options.add(new Option(resArr[i].text,resArr[i].value));
							}
					}
			}else {
	...
	function selectRes(){
		var reValue = window.showModalDialog(path+"app/toAllResource.action", this, "dialogWidth=300px;dialogHeight=500px;scroll=auto");
...
<BZ:form name="srcForm" method="post" token="menuAdd">
...
<td></td>
<td>父菜单</td>
<td><BZ:input field="PNAME" id="PNAME" type="String" prefix="DS_" defaultValue="<%=PNAME %>" onclick="showModuleTree2(this)"/></td>
<td>是否左侧显示下级菜单</td>
<td><BZ:select field="IS_LEFT" formTitle="显示" property="data" prefix="P_"><BZ:option value="1">是</BZ:option><BZ:option value="0">否</BZ:option></BZ:select></td>
...
<td>菜单对应资源</td>
<td>
		<BZ:select id="P_RES_ID" field="RES_ID" formTitle="" prefix="P_" >
...
<td></td>
<td>帮助文件路径</td>
<td><BZ:input field="HELP_FILE_PATH" type="String" prefix="P_" defaultValue="" size="40"/></td>
<td>是否模块入口</td>
<td><BZ:select field="IS_MODULE_ENTRY" formTitle="模块入口" property="data" prefix="P_"><BZ:option value="1">是</BZ:option><BZ:option value="0">否</BZ:option></BZ:select></td>
...
<td>状态</td>
<td><BZ:select field="STATUS" formTitle="状态" property="data" prefix="P_"><BZ:option value="1">启用</BZ:option><BZ:option value="2">禁用</BZ:option></BZ:select></td>
