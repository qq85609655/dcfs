...
<BZ:script isList="true"/>
	...
	var roleValue=parent.left1Frame.document.getElementById("APP_IDS").value;
	if(roleValue==null||roleValue==""){
		...
		document.srcForm.action = "<%=request.getContextPath() %>/role/Authorize!personAppAuthorize.action";
			document.srcForm.submit();
		}else{
			return;
		}
	...
	function queryAudit(){
			...
			return;
				...
				var flag = window.showModalDialog("<%=request.getContextPath() %>/role/Authorize!perAppFrame.action?PERSON_IDS="+ID, this, "dialogWidth=600px;dialogHeight=350px;scroll=auto");
				if(flag == "true"){
					document.srcForm.submit();
				}
			}
...
<BZ:form name="srcForm" method="post" isContextPath="true" action='<%="CHECK_AUDIT".equals(checkAudit)?"person/Person!queryNoRolesPersons.action":"person/Person!queryPersonAppList.action?ORG_ID=0" %>'>
...
<BZ:th name="应用权限" sortType="none" width="42%" sortplan="" sortfield="" />
</BZ:thead>
<BZ:tbody>
...
<td><BZ:data field="ROLE_CNAME" defaultValue=""/></td>
</tr>
</BZ:for>
</BZ:tbody>
</BZ:table>
