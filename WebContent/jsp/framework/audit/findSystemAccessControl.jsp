...
<BZ:script isList="true" isDate="true"/>
  ...
  $(document).ready(function(){
	...
	function search() {
		document.srcForm.action=path+"audit/findSystemAccessControl.action";
   ...
   function _export()
   {
	   if(confirm('确认要导出为Excel文件吗?')){
		   document.srcForm.action=path+"audit/exportAccessControlExcel.action";
		   document.srcForm.submit();
		   document.srcForm.action=path+"audit/findSystemAccessControl.action";
	   }
   }
...
<BZ:form name="srcForm" method="post" action="audit/findSystemAccessControl.action">
...
<BZ:thead theadclass="titleBackGrey">
	...
	<BZ:th name="行为者IP" sortType="string" width="10%" sortplan="ACTOR_IP"/>
	<BZ:th name="行为者名称" sortType="string" width="10%" sortplan="ACTOR_NAME"/>
	<BZ:th name="操作行为" sortType="string" width="10%" sortplan="ACT_ACTION"/>
	<BZ:th name="操作对象分类" sortType="string" width="10%" sortplan="ACT_OBJ_TYPE"/>
	<BZ:th name="操作对象" sortType="string" width="10%" sortplan="ACT_OBJ"/>
	<BZ:th name="操作结果" sortType="string" width="10%" sortplan="ACT_RESULT"/>
	<BZ:th name="行为时间" sortType="none" width="10%" sortplan="ACT_TIME"/>
	<BZ:th name="级别" sortType="string" width="5%" sortplan="ACT_LEVEL"/>
	<BZ:th name="备注" sortType="string" width="12%" sortplan="MEMO"/>
</BZ:thead>
<BZ:tbody>
