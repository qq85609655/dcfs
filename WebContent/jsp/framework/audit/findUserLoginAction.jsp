...
<BZ:script isList="true" isDate="true"/>
	...
	function search() {
		document.srcForm.action=path+"audit/findUserLoginAction.action";
		document.srcForm.submit();
	}

	function statisticLogin() {
		var s = document.getElementById("ACT_START_TIME").value;
		var e = document.getElementById("ACT_END_TIME").value;
		window.open(path+"audit/statisticLogin.action?ACT_START_TIME="+s+"&ACT_END_TIME="+e);
	}

	function _export()
	{
		if(confirm('确认要导出为Excel文件吗?')){
			document.srcForm.action=path+"audit/exportUserLoginExcel.action";
			document.srcForm.submit();
			document.srcForm.action=path+"audit/findUserLoginAction.action";
		}
	}
...
<BZ:form name="srcForm" method="post" action="audit/findUserLoginAction.action">
...
<tr>
	...
	<td width="20%"><input type="text" name="SYSTEM_ACTOR_NAME" value="<%=map.get("actor_name")%>"/></td>
	...
	<tr>
	<td>行为时间：</td>
	<td colspan="3">从<input size="15" class="Wdate" id="ACT_START_TIME" name="ACT_START_TIME" readonly="readonly" value="<%=map.get("start_act_time") %>" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'ACT_END_TIME\')||\'2020-10-01\'}'})"/>到<input size="15" class="Wdate" id="ACT_END_TIME"  name="ACT_END_TIME"  readonly="readonly" value="<%=map.get("end_act_time") %>" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'ACT_START_TIME\')}',maxDate:'2020-10-01'})"/></td>
	<td>数据范围：</td>
	<td><select name="ACT_TBL_NAME" id="ACT_TBL_NAME">
		<option value="PUB_AUDIT_SYSTEM">当前数据</option>
		<option value="PUB_AUDIT_SYSTEM_ARCHIVE">归档数据</option>
	</select>
	</td>
	<td align="center" valign="middle">
		<input type="button" value="查询" class="button_search" onclick="search()"/>&nbsp;&nbsp;
		<input type="button" value="统计" class="button_search" onclick="statisticLogin()"/>&nbsp;&nbsp;
...
<BZ:thead theadclass="titleBackGrey">
	...
	<BZ:th name="行为者IP" sortType="string" width="8%" sortplan="database" sortfield="ACTOR_IP"/>
	<BZ:th name="行为者名称" sortType="string" width="17%" sortplan="database" sortfield="ACTOR_NAME"/>
	...
	<BZ:th name="行为时间" sortType="string" width="15%" sortplan="database" sortfield="ACT_TIME"/>
	<BZ:th name="级别" sortType="string" width="10%" sortplan="database" sortfield="ACT_LEVEL"/>
	<BZ:th name="备注" sortType="string" width="20%" sortplan="database" sortfield="MEMO"/>
</BZ:thead>
<BZ:tbody>
