...
Data send = (Data)request.getAttribute("send");
String replyNo = (String)request.getAttribute("replyNo");
String reciverName = (String)request.getAttribute("reciverName");
String orgName_personName = (String)request.getAttribute("orgName_personName");
String smsReplyMatchLen = (String)request.getAttribute("smsReplyMatchLen");
String[] reciverTypes = reciverName.split("#");
StringBuffer buffer = new StringBuffer();
String temp = "";
if(reciverTypes.length>=0){
	for(int i=reciverTypes.length-1;i>=0;i--){
	if(!"".equals(reciverTypes[i])&&reciverTypes[i]!=null){
		String[] reciverNames = reciverTypes[i].split(";");
		if(reciverNames.length<=1){
			String[] strTemp = reciverNames[0].split("&");
			temp = "<span abc='"+strTemp[1]+"' title='点击选择短信接收人gr'><img src=\""+path+"/jsp/framework/shortinfo/images/users.gif\">"+strTemp[0]+"]&nbsp;<img src=\""+path+"/resources/resource1/images/stop.png\" title='删除' style='cursor:hand;height:10px' onclick='_deleteReciever(this);'>;</span>";
			buffer.append(temp);
		}else{
			for(int j=0;j<reciverNames.length;j++){
				String[] strTemp = reciverNames[j].split("&");
				temp = "<span abc='"+strTemp[1]+"' title='点击选择短信接收人z'><img src=\""+path+"/jsp/framework/shortinfo/images/user.gif\">"+strTemp[0]+"&nbsp;<img src=\""+path+"/resources/resource1/images/stop.png\" title='删除' style='cursor:hand;height:10px' onclick='_deleteReciever(this);'>;</span>";
				buffer.append(temp);
			}
		}
	}
...
String spans = (String)request.getAttribute("spans");
%>
...
function _onload(){
	var reply = '<%=send.getString(Sms_Send.NEED_REPLY)%>';
	var content = '<%=send.getString(Sms_Send.CONTENT)%>';
	if(reply=='1'){
		var smsReplyMatchLen = '<%=smsReplyMatchLen%>';		
		content = content.substr(0,content.length-10-smsReplyMatchLen);
		//alert(content);
		document.getElementsByName("Send_CONTENT")[0].value=content;
		_view();
		document.getElementById("view").innerHTML = document.getElementById("view").innerHTML+"<span id='replayId'>[请输入<%=replyNo%>+内容回复]</span>";
	}else{
		document.getElementsByName("Send_CONTENT")[0].value=content;
		_view();
	}
...
<table style="width: 100%;margin-top: 10px;" cellspacing="1" class="style1">
	...
	<tr>
		...
		<table style="width:100%;height:30px;" class="style4">
			<tr>
				...
				<td bgcolor="#ffffff" width="90%"><div id="shoujian" style="height:80px;overflow:auto;"><%=buffer.toString() %></div></td>
			</tr>
		</table>
		</td>
	</tr>
	...
	<tr>
		...
		<table style="width: 100%" cellspacing="0" cellpadding="0">
			<tr>
				<td>
					<textarea style="width:99%;height:250px" onkeyup="_calc(this)" name="Send_CONTENT" rows="1" cols="20"></textarea>
				</td>
	...
	<tr>
		...
		<table style="width: 100%" class="style5" border="0">
			<tr>
				...
				<td nowrap="nowrap" class="style2" style="width:60px;"><input onclick="sm(this)" type="checkbox" <%="1".equals(send.getString(Sms_Send.NEED_SIGN))?"checked='checked'":"" %>><input type="hidden" name="Send_NEED_SIGN" value="0" id="Send_NEED_SIGN" /><span>署名</span></td>
				<td nowrap="nowrap" class="style2" style="width:320px;">
					<input onclick="sel(this)" type="checkbox" id="replay_CheckId" <%="1".equals(send.getString(Sms_Send.NEED_REPLY))?"checked='checked'":"" %>><input type="hidden" name="Send_NEED_REPLY" value="0" id="Send_NEED_REPLY"/>
