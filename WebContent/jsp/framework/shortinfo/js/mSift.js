...
mSift.prototype={
	...
	Select:function(){
		...
		_this.Complete();
		var mobileName =  _this.Target.value;
		var mobile = mobileName.split('(')[0];
		var nameBack = mobileName.split('(')[1];
		var name = nameBack.substring(0,nameBack.length-1);
		var nameMobile = name+"("+mobile+")";
		var abc = ""+","+name+","+mobile+","+""+","+"";
		var returnValue = "<span abc='"+abc+"'><img src='"+path+"jsp/shortinfo/images/user.gif' />";
		returnValue += nameMobile+"<img src='"+path+"resources/resource1/images/stop.png' style='cursor:hand;height:10px' onclick='_deleteReciever(this);'/></span>";
		//alert(returnValue);
	...
	Get:function(){
		...
		if(cResult==''){_this.Clear();}
			...
			_this.TgList.style.width=_this.Target.offsetWidth-1+'px';
		}
