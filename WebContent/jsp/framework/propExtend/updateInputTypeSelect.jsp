...
<%
	String inputType = (String)request.getAttribute("INPUT_TYPE");
	if(inputType==null){
		inputType="";
	}
%>
...
<BZ:head>
	...
	function checkCode(){
		...
		var propCode = parent.document.getElementById('PROP_CODE').value.trim();
		var propCodeOld = parent.document.getElementById('PROP_CODE_OLD').value;
		if(propCode.trim() == propCodeOld.trim()){
			return true ;
		}
	...
	function next(){
			...
			document.srcForm.action=path+"prop/propExtend!gotoInputTypeNextModify.action";
...
<BZ:body onload="init();">
	<BZ:form name="srcForm" method="post">
		...
		<input type="hidden" id="view" name="view" value="<%=request.getParameter("view") %>"/>
