...
<%
    ...
    String cui_id=(String)request.getAttribute("CUI_ID");
    DataList basicDetail=(DataList)request.getAttribute("updateDetail");
		...
		<script>
			...
			function _goback(update_type){
				if(update_type=='1'){
					...
					window.location.href=path+'/cms/childupdate/updateListZX.action';
				}
			}
			//如果材料在中心需要选择单位
				...
				if(document.getElementById("smallTypeCode").value==""||document.getElementById("smallTypeCode").value=="null"){
				 ...
				 //先判断所有的更新项目是不是已经都存在，如果都存在则不允许再添加新的更新项目
				  ...
				  if(i==smTypeSigle.length){
					...
					var newTr=$("<tr>");
					var newHtml="<td align='center'><select name='smalltype' id='s"+firstCode+"' onchange='_changeSmType(this);'>";
		 ...
		 //更新项目下拉选项内容改变事件
		   ...
		   function _initData(){
				...
				var smTypeUsed=document.getElementById("smTypeUsed").value.split(',');
				var updateTypes=document.getElementById("updateTypes").value.split(',');
				var smTypeUsedStrs=new Array();
				//遍历数组smTypeValues，给smTypeSigle和smTypeUsedStrs初始化值
				for(var i=0;i<smTypeValues.length;i++){
					 smTypeSigle[i]='0';
                     for(var j=0;j<smTypeUsed.length;j++){
                         if(smTypeValues[i]==smTypeUsed[j]){
                        	 smTypeUsedStrs[j]=smTypeStrs[i];
                        	 smTypeSigle[i]='1';
                        	 break;
                          }
                      }
				...
				if(document.getElementById("smTypeUsed").value!="null"&&document.getElementById("smTypeUsed").value!=""){
	                ...
	                for(var i=0;i<smTypeUsed.length;i++){
	                    ...
	                    var firsStr=smTypeUsedStrs[i];
	                    var updateType=updateTypes[i];//更新类型 0为覆盖 1为追加
	                    var newTr=$("<tr>");
	     				var newHtml=("<td align='center'><input type='hidden' name='smalltype' value='"+firsCode+"' id='s"+firsCode+"' />"+firsStr);
	     				newHtml+=("</td><td>"+"<iframe src='"+attPath+"?packageId="+packageId+"&uploadId="+"a"+firsCode+"&smallType="+firsCode+"&org_af_id="+org_af_id+"' id='"+"a"+firsCode+"' style='height:90px;width:100%;' frameborder='0' marginwidth='0' marginheight='0'></iframe>"+"</td>");
	                    if(updateType=='1'){
	                    	newHtml+=("<td><select name='updatetype'><option value='1'>追加</option><option value='0'>覆盖</option></select></td>");
	                    }else{
	                    	newHtml+=("<td><select name='updatetype'><option value='0'>覆盖</option><option value='1'>追加</option></select></td>");
	                    }
	     				newHtml+=("<td><input type='button' name='"+firsCode+"' value='删除' class='btn btn-sm btn-primary' onclick='_deleteRow(this);'/></td>");
	                    // alert(newHtml);
	     				newTr.html(newHtml);
	     				$("#attTable").append(newTr);
	                 }
		 ...
		 //删除更新项目事件：基本信息更新
		   ...
		   function _addBasicInfo(){
			...
			function _initBasicData(){
               <% for(int i=0;i<basicDetail.size();i++){
            	   Data detail=basicDetail.getData(i);
            	   String updateField=detail.getString("UPDATE_FIELD");%>
            	   var selectValue="<%=updateField%>";
            	   var trId="T"+selectValue;
            	   var newTr=$("<tr id='"+trId+"'>");
                	 var td1=$("#"+selectValue+"1").html();
                	 var td2=$("#"+selectValue+"2").html();
			...
			<div class='panel-container'>
				<div id="tab1">
				  ...
				  <div class="bz-edit-data-content clearfix"  >
					    <table  id="basicInfo" class="table table-striped table-bordered table-hover dataTable" adsorb="both" init="false" >
						<thead>
							<tr>
								<th style="width: 15%;">
									<div >更新项目</div>
