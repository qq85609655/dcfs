...
<script type="text/javascript">
	...
	//检索
	function _search(){
		document.srcForm.action=path+"/cms/childManager/twinslist.action";
	...
	function _add(){
		...
		if(confirm('确认将选中的儿童设为同胞记录吗?')){
			 ...
			 document.srcForm.action=path+"/cms/childManager/twinsadd.action";
			 document.srcForm.submit();
			 document.srcForm.action=path+"/cms/childManager/twinslist.action";
		}else{
			return;
	...
	function _delete(cid,cno,IS_MAIN){
		document.getElementById("cid").value=cid;
		document.getElementById("cno").value=cno;
		document.getElementById("IS_MAIN").value=IS_MAIN;
		
		document.srcForm.action=path+"/cms/childManager/twinsdelete.action";
		document.srcForm.submit();
		document.srcForm.action=path+"/cms/childManager/twinslist.action";
	}
...
<BZ:body property="data" codeNames="ETXB">
	...
	<table class="specialtable" align="center" style='width:100%;text-align:center'>
		...
		<tr>
			...
			<td align="center" class="edit-data-value"><a href="javascript:_delete('<BZ:data field="CI_ID" onlyValue="true"/>','<BZ:data field="CHILD_NO" onlyValue="true"/>','<BZ:data field="IS_MAIN" onlyValue="true"/>')">删除</a></td>
		</tr>
		</BZ:for>
	</table>
	</BZ:form>
	<br>
	<!--显示同胞信息:end-->
    <BZ:form name="srcForm" method="post" action="/cms/childManager/twinslist.action">
		<!-- 查询条件区Start -->
	...
	<input type="hidden" name="IS_MAIN" id="IS_MAIN" value=""/>
