...
<BZ:body property="data" codeNames="PROVINCE;ETXB;ETSFLX;BCZL;CHILD_TYPE;ETCLSHJG_ST;BCCD">
<script type="text/javascript">
	...
	function _chgAuditOption(){		
		...
		}else if("<%=ChildStateManager.CHILD_AUD_OPTION_SUCCESS%>"==audit_option||"<%=ChildStateManager.CHILD_AUD_OPTION_FAILURE%>"==audit_option){//通过/不通过
			$("#is_modify").hide();
			$("#notice_content").hide();
			$("#audit_content").show();
			if("<%=ChildStateManager.CHILD_AUD_OPTION_FAILURE%>"==audit_option){//如果审核结果为不通过则不需要显示处理方式
				$("#audit_result").hide();
		    }else{
		    	$("#audit_result").show();	
			}
	...
	function _cancle(){
		if(confirm('确认要取消当前填写的数据么?')){
			document.srcForm.action=path+"/cms/childManager/azbAuditList.action?CHILD_TYPE=<%=CHILD_TYPE%>";
			document.srcForm.submit();
		}else{
			return;
		}
...
<input type="hidden" name="A_AUDIT_LEVEL" id="A_AUDIT_LEVEL" value="<%=ChildInfoConstants.LEVEL_CCCWA%>">

<!--补充-->
<input type="hidden" name="ADD_SOURCE" id="ADD_SOURCE" value="<%=ChildInfoConstants.LEVEL_CCCWA%>">
<input type="hidden" name="ADD_IS_FLY" id="ADD_IS_FLY" value="0">
...
<div id="tab-container" class='tab-container'>
	<ul class='etabs'>
		...
		<li class='tab'><a href="<%=path%>/cms/childManager/getChildAuditRecorderByCIID.action?UUID=<%=ciId%>" data-target="#tab4">审核记录</a></li>
	...
	<div class='panel-container'>
		...
		<table class="specialtable" align="center" style='width:98%;text-align:center'>
			...
			<tr>
				...
				<td class="edit-data-value">				
				<BZ:select prefix="P_" field="SN_DEGREE" id="P_SN_DEGREE" isCode="true" codeName="BCCD" formTitle="病残程度" defaultValue="1" width="135px" notnull="病残程度不能为空">
					<BZ:option value="">--请选择--</BZ:option>
				</BZ:select>				
				</td>
				<td class="edit-data-title">是否特别关注</td>
				<td class="edit-data-value" colspan="2">				
				<BZ:select prefix="P_" field="SPECIAL_FOCUS" id="P_SPECIAL_FOCUS" isCode="false" formTitle="是否特别关注" defaultValue="0" width="135px" notnull="是否特别关注">
					<BZ:option value="">--请选择--</BZ:option>
					<BZ:option value="0">否</BZ:option>
					<BZ:option value="1">是</BZ:option>
				</BZ:select>				
...
<div id="tab-audit">
	<table class="specialtable" align="center" style="width:96%;text-align:center">
		...
		<tr id="audit_result">
			...
			<%if(ChildInfoConstants.CHILD_TYPE_SPECAL.equals(CHILD_TYPE)){%>
			<input type="radio" name="result" id="result" checked value="<%=ChildStateManager.CHILD_AUD_RESULT_TRAN%>">送翻&nbsp;
		...
		<tr id="is_modify" style="display:none">
			<td  class="edit-data-title">发送对象</td>
			<td  class="edit-data-value">&nbsp;
			<input type="checkbox" name="ADD_IS_PROVICE" id="ADD_IS_PROVICE">&nbsp;省厅&nbsp;
			<input type="checkbox" name="ADD_IS_WELFARE" id="ADD_IS_WELFARE">&nbsp;福利院
			</td>
			...
			<td  class="edit-data-value" colspan="2">
			<input type="checkbox" name="IS_SMS" id="IS_SMS" value="">&nbsp;是否发送短信
			</td>
		</tr>
