...
<script type="text/javascript">
	...
	function _save(){
		...
		if (!runFormVerify(document.srcForm, false)) {
			...
			if(confirm('确认要保存当前填写的数据么?')){
				...
				document.getElementById("T_TRANSLATION_STATE").value = "<%=ChildInfoConstants.TRANSLATION_STATE_DOING%>";
				document.srcForm.action=path+"/cms/childTranslation/save.action";
	...
	function _submit(){
		if (!runFormVerify(document.srcForm, false)) {
			...
			if(confirm('确认要提交当前填写的数据么?')){
				...
				if(document.getElementById("P_IS_PLAN")!=null && document.getElementById("P_IS_PLAN").checked)
					document.getElementById("P_IS_PLAN").value = "1";
				document.getElementById("T_TRANSLATION_STATE").value = "<%=ChildInfoConstants.TRANSLATION_STATE_DONE%>";
				document.srcForm.action=path+"/cms/childTranslation/save.action";
				document.srcForm.submit();
			}else{
				return;
	...
	function _toipload(fn){
		...
		//alert(document.uploadForm.PACKAGE_ID.value);
...
<!-- 身份证号、儿童身份、送养人、送样人地址、捡拾日期、报刊公告、公告日期、报刊名称不需显示 -->
<BZ:input type="hidden" prefix="P_" field="ID_CARD"	id="P_ID_CARD" defaultValue=""/>
<BZ:input type="hidden" prefix="P_" field="CHILD_IDENTITY"	id="P_CHILD_IDENTITY" defaultValue=""/>
<BZ:input type="hidden" prefix="P_" field="SENDER"	id="P_SENDER" defaultValue=""/>
<BZ:input type="hidden" prefix="P_" field="SENDER_ADDR"	id="P_SENDER_ADDR" defaultValue=""/>
<BZ:input type="hidden" prefix="P_" field="PICKUP_DATE"	id="P_PICKUP_DATE" defaultValue=""/>
<BZ:input type="hidden" prefix="P_" field="IS_ANNOUNCEMENT"	id="P_IS_ANNOUNCEMENT" defaultValue=""/>
<BZ:input type="hidden" prefix="P_" field="ANNOUNCEMENT_DATE"	id="P_ANNOUNCEMENT_DATE" defaultValue=""/>
<BZ:input type="hidden" prefix="P_" field="NEWS_NAME"	id="P_NEWS_NAME" defaultValue=""/>
...
<table class="specialtable" align="center" style='width:98%;text-align:center'>
	...
	<tr>
		...
		<td class="edit-data-value"><BZ:input prefix="P_" field="NAME" id="P_NAME" notnull="姓名不能为空！" defaultValue="" size="25"/></td>
	...
	<tr>
		<td class="edit-data-title">性别<font color="red">*</font></td>
		<td class="edit-data-value">
		<BZ:select prefix="P_" field="SEX" id="P_SEX" isCode="true" codeName="ETXB" formTitle="性别" width="45%" notnull="性别不能为空！">
			<option value="">--请选择--</option>
		</BZ:select></td>
		<td class="edit-data-title">出生日期<font color="red">*</font></td>
		<td class="edit-data-value"><BZ:input prefix="P_" field="BIRTHDAY" id="P_BIRTHDAY" type="date" notnull="出生日期不能为空！" /></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class="edit-data-value"><BZ:input type="date" prefix="P_" field="ENTER_DATE" id="P_ENTER_DATE" defaultValue=""/></td>
		<td class="edit-data-title">报送日期</td>
		<td class="edit-data-value"><BZ:input type="date" prefix="P_" field="SEND_DATE" id="P_SEND_DATE" defaultValue=""/>		
		</td>
	</tr>
	...
	<tr>
		...
		<td class="edit-data-value" colspan="4">
		<textarea name="P_DISEASE_EN" id="P_DISEASE_EN" rows="5" cols="100" maxlength="1000"><BZ:dataValue field="DISEASE_EN" defaultValue="" onlyValue="true"/></textarea>
		</td>
	</tr>
	<%}%>
	<tr>
	...
	<tr>
		<td colspan="5">
		<IFRAME ID="frmUpload1" SRC="<%=path%>/common/batchattmaintain.action?bigType=CI&packID=<%=packId%>&packageID=<%=fciId%>" frameborder=0 width="100%" height="100%"></IFRAME> 
		</td>
	</tr>
	<!--附件：end-->
</table>
...
<div id="tab-translation">
	<table class="specialtable" align="center" style="width:98%;text-align:center">
		...
		<tr>
			...
			<td  class="edit-data-value">
			<BZ:input prefix="T_" field="TRANSLATION_DESC" id="R_TRANSLATION_DESC" type="textarea" formTitle="翻译说明" defaultValue="" maxlength="500" style="width:80%"/>
			</td>
		</tr>
	</table>
</div>
