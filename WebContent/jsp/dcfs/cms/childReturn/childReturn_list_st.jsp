...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"cms/childreturn/returnListST.action?page=1";
		...
		function _childReturn(){
			var url = path + "cms/childreturn/returnSelectST.action";
		...
		function _toReturn(CI_ID){
			document.srcForm.action=path+"cms/childreturn/toReturnAdd.action?CI_ID="+CI_ID+"&RETURN_LEVEL=2";
			document.srcForm.submit();
		}
		//进入退材料确认页面
		function _confirm(){
			...
			if(num != "1"){
				...
				if(RETURN_STATE!="1"){
				   page.alert("只能确认退材料状态为待省厅确认的退材料信息，请重新选择！");
				   return;
			    }
				document.srcForm.action=path+"cms/childreturn/toConfirm.action?AR_ID="+AR_ID+"&CONFIRM_LEVEL=2";
				document.srcForm.submit();
			}
		}
	...
	<BZ:body property="data" codeNames="ETXB;TCLFLST;">
		<BZ:form name="srcForm" method="post" action="cms/childreturn/returnListST.action">
			...
			<div class="wrapper">
				...
				<div class="footer-frame">
					<table border="0" cellpadding="0" cellspacing="0" class="operation">
						<tr>
							<td><BZ:page form="srcForm" property="List"  exportXls="true" exportTitle="儿童材料退材料" exportCode="SEX=CODE,ETXB;BIRTHDAY=DATE;APPLE_DATE=DATE;BACK_TYPE=CODE,TCLFLST;BACK_DATE=DATE;BACK_RESULT=FLAG,0:未确认&1:已确认;" exportField="WELFARE_NAME_CN=福利院,20,20;NAME=姓名,10,20;SEX=性别,8;BIRTHDAY=出生日期,10;APPLE_DATE=申请日期,10;BACK_TYPE=退文类型,15;RETURN_REASON=退材料原因,50;BACK_DATE=退材料日期,15;BACK_RESULT=状态,15;"/></td>
						</tr>
					</table>
				</div>
				<!--分页功能区End -->
