<%
/**   
 * @Title: AZQ_PP_feedback_translation_list.jsp
 * @Description: 爱之桥安置后报告翻译列表
 * @author xugy
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"feedback/AZQPPFeedbackTransList.action?page=1";
		...
		function _reset(){
			...
			document.getElementById("S_DISTRIB_DATE_START").value = "";
			document.getElementById("S_DISTRIB_DATE_END").value = "";
		...
		function _translation(){
			...
			for(var i=0; i<arrays.length; i++){
				if(arrays[i].checked){
					...
					var TRANSLATION_UNIT = id.split(",")[2];
					if(TRANSLATION_UNIT != "" && TRANSLATION_UNIT != "91"){
						page.alert('选择的翻译数据已分发，请重新选择');
						return;
					}
					ids=id.split(",")[0];
					num += 1;
			...
			if(num != 1){
				...
				document.srcForm.action=path+"feedback/toAZQPPFeedbackTranslation.action";
				document.srcForm.submit();
			}
		}
		//分发
		...
		//文件分发确认
		function _selDispatchUnit(){	
			$('#TRANSLATION_UNIT').val($("#unit").val());
			$('#TRANSLATION_UNITNAME').val($("#unit").find("option:selected").text());
			document.srcForm.action=path+"feedback/dispatch.action";
			document.srcForm.submit();
		}
		//查看
		function _detail(){
			...
			if(num != 1){
				...
				document.getElementById("ids").value = ids;
				document.srcForm.action=path+"feedback/AZQPPFeedbackTranslationDetail.action";
				document.srcForm.submit();
			}
		}
	</script>
	<BZ:body property="data" codeNames="FYDW;GJSY;SYS_GJSY_CN;">
		<BZ:form name="srcForm" method="post" action="feedback/AZQPPFeedbackTransList.action">
					...
					<td style="width: 100%;">
						<table>
							...
							<tr>
								...
								<td class="bz-search-title">分发日期</td>
								<td colspan="3">
									<BZ:input prefix="S_" field="DISTRIB_DATE_START" id="S_DISTRIB_DATE_START" type="Date" dateExtend="maxDate:'#F{$dp.$D(\\'S_DISTRIB_DATE_END\\')}',readonly:true" defaultValue="" formTitle="起始分发日期" />~
									<BZ:input prefix="S_" field="DISTRIB_DATE_END" id="S_DISTRIB_DATE_END" type="Date" dateExtend="minDate:'#F{$dp.$D(\\'S_DISTRIB_DATE_START\\')}',readonly:true" defaultValue="" formTitle="截止分发日期" />
								</td>
							</tr>
							<tr>
		...
		<!-- 查询条件区End -->
		<!--选择分发单位:start-->
		<div class="table-row" id="dispatchDiv" style="display: none;width: 100%;">
			<table cellspacing="0" cellpadding="0" width="100%" >
				<tr>
					<td style="width:100%;">
						<table style="width:100%">
						  <tr>
						  	<td class="bz-search-title" style="width: 35%">单位名称</td>
						  	<td style="width:65%">
								<BZ:select field="TRANSLATION_UNIT" id="unit" isCode="true" formTitle="单位名称" codeName="FYDW" notnull="" width="327px"/>
							</td>																			
						  </tr>
						</table>
					</td>
				...
				<tr>
					<td><font color="ff0000">说明：选择分发单位并分发成功后，该文件只能由被分发的单位进行翻译，同时重翻、补翻的文件直接转到该单位进行翻译。</font></td>
				</tr>
				<tr style="height: 17px;"></tr>
				<tr>
					<td style="text-align: center;">
						<div class="bz-search-button">
							<input type="button" value="确&nbsp;&nbsp;定" onclick="_selDispatchUnit()" class="btn btn-sm btn-primary">
						</div>
					</td>
				</tr>
			</table>
		...
		<div class="page-content">
			<div class="wrapper">
				...
				<div class="table-row table-btns" style="text-align: left">
					...
					<input type="button" value="分&nbsp;&nbsp;发" class="btn btn-sm btn-primary" onclick="_distrib()"/>&nbsp;
					...
					<table class="table table-striped table-bordered table-hover dataTable" adsorb="both" init="true" id="sample-table">
						<thead>
							<tr>
								...
								<th style="width: 5%;">
									<div class="sorting" id="DISTRIB_DATE">分发日期</div>
								</th>
								<th style="width: 5%;">
						...
						<BZ:for property="List">
							<tr class="emptyData">
								<td class="center">
									<input name="xuanze" type="checkbox" value="<BZ:data field="FB_T_ID" defaultValue="" onlyValue="true"/>,<BZ:data field="TRANSLATION_STATE" defaultValue="" onlyValue="true"/>,<BZ:data field="TRANSLATION_UNIT" defaultValue="" onlyValue="true"/>" class="ace">
								</td>
								<td class="center">
									<BZ:i/>
								</td>
								...
								<td><BZ:data field="DISTRIB_DATE" defaultValue="" type="date"/></td>
								<td><BZ:data field="COMPLETE_DATE" defaultValue="" type="date"/></td>
								<td><BZ:data field="TRANSLATION_UNITNAME" defaultValue="" /></td>
				...
				<div class="footer-frame">
					<table border="0" cellpadding="0" cellspacing="0" class="operation">
						<tr>
							<td><BZ:page form="srcForm" property="List" exportXls="true" exportTitle="安置后反馈报告翻译数据" exportCode="TRANSLATION_STATE=FLAG,0:待翻译&1:翻译中&2:已翻译;REPORT_DATE=DATE;RECEIVER_DATE=DATE;DISTRIB_DATE=DATE;COMPLETE_DATE=DATE;SIGN_DATE=DATE,yyyy/MM/dd" exportField="ARCHIVE_NO=档案号,15,20;COUNTRY_CN=国家,15;NAME_CN=收养组织,20;MALE_NAME=男方,15;FEMALE_NAME=女方,15;NAME=儿童姓名,15;SIGN_DATE=签批日期,15;NUM=次第数,10;REPORT_DATE=反馈日期,15;RECEIVER_DATE=交接日期,15;DISTRIB_DATE=分发日期,15;COMPLETE_DATE=完成日期,15;TRANSLATION_UNITNAME=翻译单位,15;TRANSLATION_STATE=翻译状态,15;"/></td>
						</tr>
					</table>
				</div>
				<!--分页功能区End -->
