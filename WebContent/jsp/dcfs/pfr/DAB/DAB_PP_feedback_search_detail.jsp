<%
/**   
 * @Title: DAB_PP_feedback_search_detail.jsp
 * @Description: 档案部安置后报告查看
 * @author xugy
 * @date 2014-12-1下午4:27:23
 * @version V1.0   
 */
%>
...
//
function _goback(){
	document.srcForm.action=path+"feedback/DABPPFeedbackSearchList.action";
...
<BZ:form name="srcForm" method="post">
	<div id="tab-container" class='tab-container'>
		...
		<div class='panel-container'>
			...
			%>
			<div id="tab1">
				<iframe id="FRFrame1" name="FRFrame1" class="FRFrame1" frameborder=0 style="width: 100%;height: 800px;" src="<%=path%>/feedback/DABPPFeedbackRecordSearchDetail.action?FEEDBACK_ID=<%=FEEDBACK_ID%>&NUM=1&BIRTHDAY=<%=BIRTHDAY%>"></iframe>
			</div>
			<%
			}
			if(num2){
			%>
			<div id="tab2">
				<iframe id="FRFrame2" name="FRFrame2" class="FRFrame2" frameborder=0 style="width: 100%;height: 800px;" src="<%=path%>/feedback/DABPPFeedbackRecordSearchDetail.action?FEEDBACK_ID=<%=FEEDBACK_ID%>&NUM=2&BIRTHDAY=<%=BIRTHDAY%>"></iframe>
			</div>
			<%
			}
			if(num3){
			%>
			<div id="tab3">
				<iframe id="FRFrame3" name="FRFrame3" class="FRFrame3" frameborder=0 style="width: 100%;height: 800px;" src="<%=path%>/feedback/DABPPFeedbackRecordSearchDetail.action?FEEDBACK_ID=<%=FEEDBACK_ID%>&NUM=3&BIRTHDAY=<%=BIRTHDAY%>"></iframe>
			</div>
			<%
			}
			if(num4){
			%>
			<div id="tab4">
				<iframe id="FRFrame4" name="FRFrame4" class="FRFrame4" frameborder=0 style="width: 100%;height: 800px;" src="<%=path%>/feedback/DABPPFeedbackRecordSearchDetail.action?FEEDBACK_ID=<%=FEEDBACK_ID%>&NUM=4&BIRTHDAY=<%=BIRTHDAY%>"></iframe>
			</div>
			<%
			}
			if(num5){
			%>
			<div id="tab5">
				<iframe id="FRFrame5" name="FRFrame5" class="FRFrame5" frameborder=0 style="width: 100%;height: 800px;" src="<%=path%>/feedback/DABPPFeedbackRecordSearchDetail.action?FEEDBACK_ID=<%=FEEDBACK_ID%>&NUM=5&BIRTHDAY=<%=BIRTHDAY%>"></iframe>
			</div>
			<%
			}
			if(num6){
			%>
			<div id="tab6">
				<iframe id="FRFrame6" name="FRFrame6" class="FRFrame6" frameborder=0 style="width: 100%;height: 800px;" src="<%=path%>/feedback/DABPPFeedbackRecordSearchDetail.action?FEEDBACK_ID=<%=FEEDBACK_ID%>&NUM=6&BIRTHDAY=<%=BIRTHDAY%>"></iframe>
			</div>
			<%
			}
			%>
