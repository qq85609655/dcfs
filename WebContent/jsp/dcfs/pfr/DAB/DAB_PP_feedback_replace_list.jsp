<%
/**   
 * @Title: DAB_PP_feedback_replace_list.jsp
 * @Description: 档案部安置后报告代录列表
 * @author xugy
 * @date 2014-10-14下午5:48:34
 * @version V1.0   
 */
%>
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"feedback/DABPPFeedbackReplaceList.action?page=1";
		...
		function _replace(){
			...
			for(var i=0; i<arrays.length; i++){
				if(arrays[i].checked){
					...
					if(REPORT_STATE != "0"){
						page.alert('请选择待上报的数据');
						return;
					}
					var IS_FINISH = id.split(",")[2];
					if(IS_FINISH == "1"){
						page.alert('该收养的安置后报告已全部结束，请重新选择');
						return;
					}
					ids=id.split(",")[0];
					num += 1;
			...
			if(num != 1){
				...
				document.srcForm.action=path+"feedback/toDABPPFeedbackReplaceInto.action";
				document.srcForm.submit();
			}
		}
		//打印首页
	...
	<BZ:body property="data" codeNames="PROVINCE;GJSY;SYS_GJSY_CN;">
		<BZ:form name="srcForm" method="post" action="feedback/DABPPFeedbackReplaceList.action">
					...
					<td style="width: 100%;">
						<table>
							...
							<tr>	
								...
								<td class="bz-search-title">录入人</td>
								<td>
									<BZ:input prefix="S_" field="REG_USERNAME" id="S_REG_USERNAME" defaultValue="" formTitle="录入人" maxlength="" />
								</td>
							...
							<tr>	
								...
								<td>
									<BZ:select prefix="S_" field="REPORT_STATE" id="S_REPORT_STATE" width="148px" formTitle="报告状态" defaultValue="">
										<BZ:option value="">--请选择--</BZ:option>
										<BZ:option value="0">待上报</BZ:option>
		...
		<div class="page-content">
			<div class="wrapper">
				...
				<div class="table-row table-btns" style="text-align: left">
					...
					<input type="button" value="代&nbsp;&nbsp;录" class="btn btn-sm btn-primary" onclick="_replace()"/>&nbsp;
				...
				<div id="scrollDiv">
					<table class="table table-striped table-bordered table-hover dataTable" adsorb="both" init="true" id="sample-table">
						...
						<BZ:for property="List">
							<tr class="emptyData">
								<td class="center">
									<input name="xuanze" type="checkbox" value="<BZ:data field="FB_REC_ID" defaultValue="" onlyValue="true"/>,<BZ:data field="REPORT_STATE" defaultValue="" onlyValue="true"/>,<BZ:data field="IS_FINISH" defaultValue="" onlyValue="true"/>" class="ace">
								</td>
								<td class="center">
									<BZ:i/>
								</td>
								...
								<td><BZ:data field="REG_USERNAME" defaultValue="" /></td>
								<td><BZ:data field="REG_DATE" defaultValue="" type="date"/></td>
								<td><BZ:data field="REPORT_STATE" defaultValue="" checkValue="0=待上报;1=待接收;2=已接收;3=待翻译;4=翻译中;5=已翻译;6=待审核;7=审核中;8=已审核;"/></td>
							</tr>
						</BZ:for>
						</tbody>
					</table>
				...
				<div class="footer-frame">
					<table border="0" cellpadding="0" cellspacing="0" class="operation">
						<tr>
							<td><BZ:page form="srcForm" property="List" exportXls="true" exportTitle="安置后反馈报告数据" exportCode="PROVINCE_ID=CODE,PROVINCE;REPORT_STATE=FLAG,0:待上报&1:待接收&2:已接收&3:待翻译&4:翻译中&5:已翻译&6:待审核&7:审核中&8:已审核;REG_DATE=DATE,yyyy/MM/dd" exportField="ARCHIVE_NO=档案号,15,20;FILE_NO=收文编号,15;SIGN_NO=签批号,15;COUNTRY_CN=国家,15;NAME_CN=收养组织,15;MALE_NAME=男方,15;FEMALE_NAME=女方,15;PROVINCE_ID=省份,15;WELFARE_NAME_CN=福利院,15;NAME=儿童姓名,15;NAME_PINYIN=姓名拼音,15;CHILD_NAME_EN=入籍姓名,15;NUM=次第数,15;REG_USERNAME=录入人,15;REG_DATE=录入日期,15;REPORT_STATE=报告状态,15;"/></td>
						</tr>
					</table>
				</div>
				<!--分页功能区End -->
