...
<BZ:head>
	...
	function _save() {
		...
		if (!runFormVerify(document.srcForm, false)) {
			...
			document.srcForm.action = path + "ffs/audit/saveOpinionTem.action";
	...
	/**
	*根据选择不同的审核级别，动态转换审核意见下拉列表
	*/
	/**function _dynamicJznsy(){
		var shjb = $("#P_AUDIT_TYPE").find("option:selected").text();
		if(shjb=="经办人审核"){
			$("#P_AUDIT_OPTION").empty();
			$("#P_AUDIT_OPTION").prepend("<option value='1'>上报</option>"); 
			$("#P_AUDIT_OPTION").prepend("<option value='2'>通过</option>"); 
			$("#P_AUDIT_OPTION").prepend("<option value='3'>不通过</option>"); 
			$("#P_AUDIT_OPTION").prepend("<option value='4'>补充文件</option>"); 
			$("#P_AUDIT_OPTION").prepend("<option value='5'>补充文件翻译</option>"); 
			$("#P_AUDIT_OPTION").prepend("<option value='6'>重新翻译</option>"); 
		}else if(shjb=="部门主任复核"){
			$("#P_AUDIT_OPTION").empty();
			$("#P_AUDIT_OPTION").prepend("<option value='2'>通过</option>"); 
			$("#P_AUDIT_OPTION").prepend("<option value='3'>不通过</option>");
			$("#P_AUDIT_OPTION").prepend("<option value='7'>退回经办人</option>");
			$("#P_AUDIT_OPTION").prepend("<option value='8'>上报分管主任</option>");
		}else{
			$("#P_AUDIT_OPTION").empty();
			$("#P_AUDIT_OPTION").prepend("<option value='2'>通过</option>"); 
			$("#P_AUDIT_OPTION").prepend("<option value='3'>不通过</option>");
