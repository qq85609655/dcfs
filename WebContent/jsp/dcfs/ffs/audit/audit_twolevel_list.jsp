...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"ffs/jbraudit/findListForTwoLevel.action";
	...
	function _isCanAudit(){
		...
		if(null==xuanze||""==xuanze||"undefined"==xuanze){
			...
			if("2"!=AUDIT_STATUS){
				page.alert("请选择审核状态为部门主任[待复核]的记录！");
				return;
			}else{
				var AUDIT_LEVEL="1";
	...
	function _audit(AF_ID,AUDIT_LEVEL,OPERATION_STATE){
		$.ajax({
			...
			success: function(data){
				...
				window.location.href=path+"ffs/jbraudit/toAuditForTwoLevel.action?AF_ID="+AF_ID+"&AU_ID="+AU_ID;
			}
		  });
	}
		...
		$.ajax({
			...
			success: function(data){
				...
				$.layer({
					...
					shade : [0.5 , '#D9D9D9' , true],
					border :[2 , 0.3 , '#000', true],
					iframe: {src: '<BZ:url/>/ffs/jbraudit/toAuditForTwoLevelView.action?AF_ID='+AF_ID+'&AU_ID='+AU_ID},
	...
	<BZ:body property="data" codeNames="GJSY;SYS_GJSY_CN;WJLX;SYS_ADOPT_ORG;SYBMSH;SYWJSHZT;SYZZ;WJFYZL;SYJBRSH;SYWJBC;SYWJCF;SYBMSH;WJSHCZZT;SYWJSHZT">
		<BZ:form name="srcForm" method="post" action="ffs/jbraudit/findListForTwoLevel.action">
		<!-- 用于保存数据结果提示 -->
		...
		<div class="page-content">
			<div class="wrapper">
				...
				<div class="table-row table-btns" style="text-align: left">
					...
					<input type="button" value="复&nbsp;&nbsp;核" class="btn btn-sm btn-primary" onclick="_isCanAudit()"/>&nbsp;
				...
				<div class="footer-frame">
					<table border="0" cellpadding="0" cellspacing="0" class="operation">
						<tr>
							<td>
							<BZ:page form="srcForm" property="List" exportXls="true" exportTitle="复核审核数据" exportCode="FILE_TYPE=CODE,WJLX;COUNTRY_CODE=CODE,GJSY;ADOPT_ORG_ID=CODE,SYS_ADOPT_ORG;AUD_STATE=CODE,SYWJSHZT;SUPPLY_STATE=CODE,SYWJBC;RTRANSLATION_STATE=CODE,SYWJCF;TRANSLATION_QUALITY=CODE,WJFYZL;REGISTER_DATE=DATE;RECEIVER_DATE=DATE" exportField="FILE_NO=收文编号,15,20;REGISTER_DATE=收文日期,15;FILE_TYPE=文件类型,10;COUNTRY_CODE=国家,10;ADOPT_ORG_ID=收养组织,20;MALE_NAME=男收养人,30;FEMALE_NAME=女收养人,30;RECEIVER_DATE=接收日期,15;AUD_STATE=审核状态,15;TRANSLATION_QUALITY=翻译质量,15"/>
