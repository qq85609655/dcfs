...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"ffs/jbraudit/findListForOneLevel.action";
		...
		function _exportExcel(){
			if(confirm('确认要导出为Excel文件吗?')){
				//document.srcForm.action=path+"ffs/jbraudit/fileExportForOneLevel.action";
				//document.srcForm.submit();
				//document.srcForm.action=path+"ffs/jbraudit/findListForOneLevel.action";
	...
	function _isCanAudit(){
		...
		if(null==xuanze||""==xuanze||"undefined"==xuanze){
			...
			if("0"!=AUDIT_STATUS&&"1"!=AUDIT_STATUS&&"9"!=AUDIT_STATUS){
				page.alert("请选择审核状态为经办人[待审核][审核中][退回重审]的记录！");
				return;
			}else{
				var AUDIT_LEVEL="0";
	...
	function _audit(AF_ID,AUDIT_LEVEL,OPERATION_STATE){
		$.ajax({
			...
			success: function(data){
				...
				window.location.href=path+"ffs/jbraudit/toAuditForOneLevel.action?AF_ID="+AF_ID+"&AU_ID="+AU_ID;
			}
		  });
	}
	...
	//修改
	function _modify(){
		window.location.href=path+"ffs/jbraudit/fileauditpage.action";
	}
		...
		$.ajax({
			...
			success: function(data){
				...
				$.layer({
					...
					shade : [0.5 , '#D9D9D9' , true],
					border :[2 , 0.3 , '#000', true],
					iframe: {src: '<BZ:url/>/ffs/jbraudit/toAuditForOneLevelView.action?AF_ID='+AF_ID+'&AU_ID='+AU_ID},
	...
	<BZ:body property="data" codeNames="GJSY;SYS_GJSY_CN;WJLX;SYZZ;SYS_ADOPT_ORG;WJFYZL;SYJBRSH;SYWJBC;SYWJCF;SYWJBF;WJSHCZZT;SYWJSHZT">
		<BZ:form name="srcForm" method="post" action="ffs/jbraudit/findListForOneLevel.action" >
		<!-- 用于保存数据结果提示 -->
					...
					<td style="width: 100%;">
						<table>
							...
							<tr>
								...
								<td style="width: 18%">
									<BZ:select field="SUPPLY_STATE" formTitle=""
								...
								<td class="bz-search-title" style="width: 10%">
									<span title="补翻状态">补翻状态</span>
								</td>
								<td style="width: 18%">
									<BZ:select field="ATRANSLATION_STATE" formTitle=""
										prefix="S_" isCode="true" codeName="SYWJBF" width="70%">
										<option value="">
											--请选择--
										</option>
									</BZ:select>
		...
		<div class="page-content">
			<div class="wrapper">
				...
				<div class="table-row table-btns" style="text-align: left">
					...
					<input type="button" value="初&nbsp;&nbsp;审" class="btn btn-sm btn-primary" onclick="_isCanAudit()"/>&nbsp;
					...
					<table class="table table-striped table-bordered table-hover dataTable" adsorb="both" init="true" id="sample-table">
						<thead>
							<tr>
								...
								<th style="width: 5%;">
									<div class="sorting" id="SUPPLY_STATE">
										补充状态
									</div>
								</th>
								<th style="width: 5%;">
									<div class="sorting" id="ATRANSLATION_STATE">
										补翻状态
									</div>
								</th>
								<th style="width: 5%;">
						...
						<BZ:for property="List"  fordata="resultData">
							<tr class="emptyData">
								...
								<td>
									<BZ:data field="SUPPLY_STATE" defaultValue="" onlyValue="true" codeName="SYWJBC"/>
								</td>
								<td>
									<BZ:data field="ATRANSLATION_STATE" defaultValue="" onlyValue="true" codeName="SYWJBF"/>
								</td>
								<td>
				...
				<div class="footer-frame">
					<table border="0" cellpadding="0" cellspacing="0" class="operation">
						<tr>
							<td><BZ:page form="srcForm" property="List" exportXls="true" exportTitle="初审审核数据" exportCode="FILE_TYPE=CODE,WJLX;COUNTRY_CODE=CODE,GJSY;ADOPT_ORG_ID=CODE,SYS_ADOPT_ORG;AUD_STATE=CODE,SYWJSHZT;SUPPLY_STATE=CODE,SYWJBC;RTRANSLATION_STATE=CODE,SYWJCF;TRANSLATION_QUALITY=CODE,WJFYZL;REGISTER_DATE=DATE;RECEIVER_DATE=DATE" exportField="FILE_NO=收文编号,15,20;REGISTER_DATE=收文日期,15;FILE_TYPE=文件类型,10;COUNTRY_CODE=国家,10;ADOPT_ORG_ID=收养组织,20;MALE_NAME=男收养人,30;FEMALE_NAME=女收养人,30;RECEIVER_DATE=接收日期,15;AUD_STATE=审核状态,15;SUPPLY_STATE=补充状态,15;RTRANSLATION_STATE=重翻状态,15;TRANSLATION_QUALITY=翻译质量,15"/></td>
						</tr>
					</table>
				</div>
				<!--分页功能区End -->
