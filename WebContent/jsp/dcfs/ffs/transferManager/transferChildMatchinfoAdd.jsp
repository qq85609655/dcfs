...
<script>
	...
	function _fileSelect() {
		...
		window.open(path + "transferManager/MannualChildMatchinfo.action?mannualDeluuid="+mannualDeluuid+"&TRANSFER_CODE="+TRANSFER_CODE,"",",'height=800,width=1200,top=100,left=100,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no'");
	}
		
	
	//更新交接单
	function refreshLocalList(uuid){
		//window.location.href = path+"transferManager/addcheque.action";
		//var uuid = document.getElementById("TI_ID").value;
		var mannualDeluuid = document.getElementById("mannualDeluuid").value;
		if(mannualDeluuid != ""){
		...
		document.srcForm.action=path+"transferManager/editChildMatchinfo.action?type=refresh";
	 	document.srcForm.submit();
	}
	...
	function _remove(){
		...
		if (confirm("确定移除这些儿童材料？")){
			var uuid = chioceuuid.join("#");
			document.getElementById("chioceuuid").value=uuid;
			document.srcForm.action = path + "transferManager/removeChildMatchinfo.action";
			document.srcForm.submit();
		}
	}
</script>
<BZ:body property="Edit_Transfer_data" codeNames="WJLX;ETXB;PROVINCE;CHILD_TYPE">
