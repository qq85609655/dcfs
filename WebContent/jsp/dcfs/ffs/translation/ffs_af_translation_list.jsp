 ...
 */
  ...
  String orgCode = organ.getOrgCode().substring(0,2);
%>
<BZ:html>
	<BZ:head>
		<title>文件翻译查询列表</title>
...
function _showSearch(){
  ...
  function search(){
     document.srcForm.action=path+"/ffs/ffsaftranslation/findList.action";
  ...
  function _view(){
	...
	if(num != "1"){
	 ...
	 //document.srcForm.action=path+"/ffs/ffsaftranslation/show.action?type=mod&UUID="+showuuid;
	 //document.srcForm.submit();
...
//文件翻译
  function _translation(){
		...
		for(var i=0; i<arrays.length; i++){
			if(arrays[i].checked){
				...
				unit=document.getElementsByName('abc')[i].getAttribute("TRANSLATION_UNIT");		
				num += 1;
			}
		}
		if(num != "1"){
		...
		//	return;
		}else if("null"!=unit && unit!="<%=orgCode%>" ){
			page.alert('该文件已分发给其他单位翻译!');
			return;
		}else{
  ...
  function _selDispatchUnit(){	
		...
		document.srcForm.action=path+"/ffs/ffsaftranslation/dispatch.action";
		document.srcForm.submit();
  }
	...
	for(var i=0;i<document.getElementsByName('abc').length;i++){
	   if(document.getElementsByName('abc')[i].checked){
			uuid=uuid+"#"+document.getElementsByName('abc')[i].value;
	   		//
			var unit = document.getElementsByName('abc')[i].getAttribute("TRANSLATION_UNIT");
			var state = document.getElementsByName('abc')[i].getAttribute("TRANSLATION_STATE");
			if(("null"!=unit && ""!=unit)  || "0"!=state){//如果翻译单位不为空或翻译状态不是待翻译
				alert('请选择未分发且未翻译的文件！');
				return;
			}
	  ...
	  if(sfdj=="0"){
		...
		return;
		  ...
		  document.getElementById("dispatchuuid").value=uuid;
		  $.layer({
			type : 1,
			title : "请选择分发单位：",
			shade : [0.5 , '#D9D9D9' , true],
  ...
  <BZ:body property="data" codeNames="WJLX;GJSY;SYS_GJSY_CN;SYZZ;WJLX_DL;FYDW">
     <BZ:form name="srcForm" method="post" action="/ffs/ffsaftranslation/findList.action">
	 <!-- 查询条件区Start -->
	 <div class="table-row" id="searchDiv" style="display: none">
		...
		<!-- 查询条件区End -->
		<!--选择分发单位:start-->
		<div class="table-row" id="dispatchDiv" style="display: none" style="width:100%">
				...
				<td style="width:100%;">
					<table style="width:100%">
						  <tr>
						  	...
						  	<td style="width:65%">
								<BZ:select field="TRANSLATION_UNIT" id="unit" isCode="true" formTitle="" codeName="FYDW" notnull="" width="327px"/>
							</td>																			
						  </tr>
	 ...
	 <input type="hidden" name="S_TRANSLATION_TYPE" id="S_TRANSLATION_TYPE" value="0"/>
	 ...
	 <div class="table-row table-btns" style="text-align: left">
		...
		<%if("91".equals(orgCode)) {%>
		<input type="button" value="手动分发" class="btn btn-sm btn-primary" onclick="_dispatch()"/>	
		<input type="button" value="扫描分发" class="btn btn-sm btn-primary" onclick="_scanDispatch()"/>
		<%} %>
	...
	<!-- 功能按钮操作区End -->
				...
				<thead>
					...
					<tbody>	
						<BZ:for property="List">
							<tr class="emptyData">
								<td class="center">
									<input name="abc" type="checkbox" value="<BZ:data field="AT_ID" onlyValue="true"/>" class="ace" 
									TRANSLATION_STATE="<BZ:data field="TRANSLATION_STATE" onlyValue="true"/>" 
									IS_PAUSE="<BZ:data field="IS_PAUSE" onlyValue="true"/>" 
									RETURN_STATE="<BZ:data field="RETURN_STATE" onlyValue="true"/>" 
									<BZ:data field="IS_PAUSE"  defaultValue="" onlyValue="true" checkValue="0=;1=disabled "/> 
									<BZ:data field="RETURN_STATE"  defaultValue="" onlyValue="true" checkValue="0=disabled;1=disabled ;2=disabled ;3=disabled "/> 
									TRANSLATION_UNIT="<BZ:data field="TRANSLATION_UNIT" onlyValue="true"/>">
								</td>
								<td class="center">
									<BZ:i/>
								</td>
								...
								<td><BZ:data field="RECEIVE_DATE"  type="Date" defaultValue="" onlyValue="true"/></td>
		...
		<div class="footer-frame">
			<table border="0" cellpadding="0" cellspacing="0" class="operation">
				<tr>
					<td><BZ:page form="srcForm" property="List" exportXls="true" exportTitle="文件翻译列表" 
					...
					exportField="FILE_NO=收文编号,15,20;REGISTER_DATE=收文日期,10;FILE_TYPE=文件类型,10;COUNTRY_CN=国家,10;NAME_CN=收养机构,25;MALE_NAME=男收养人,15;FEMALE_NAME=女收养人,15;RECEIVE_DATE=接收日期,10;COMPLETE_DATE=完成日期,10;TRANSLATION_UNITNAME=翻译单位,15;TRANSLATION_STATE=翻译状态,10;"/></td>				
				</tr>
			</table>
		</div>
		<!--分页功能区End -->
