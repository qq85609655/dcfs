...
<%
/**   
 * @Title: fileinfoC_select.jsp
 * @Description: 锁定类型为C的文件信息列表
 * @author yangrt   
 * @date 2014-9-21
 * @version V1.0  
 */
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"sce/lockchild/FileInfoSelect.action?type=C&page=1";
		...
		function _showFileData(af_id){
			var url = path + "sce/lockchild/GetAdoptionPersonInfo.action?Flag=EN&type=C&AF_ID=" + af_id;
			_open(url,"window",950,600);
		}
		
	</script>
	<BZ:body property="searchData" codeNames="WJLX;SDFS;ADOPTER_CHILDREN_SEX;">
		<BZ:form name="srcForm" method="post" action="sce/lockchild/FileInfoSelect.action?type=C">
		<!-- 用于保存数据结果提示 -->
