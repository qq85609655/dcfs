...
<%
/**   
 * @Title: fileinfoA_select.jsp
 * @Description: 锁定类型为A的文件信息列表
 * @author yangrt   
 * @date 2014-9-21
 * @version V1.0  
 */
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"sce/lockchild/FileInfoSelect.action?type=A&page=1";
		...
		function _goNext(){
			...
			for(var i=0; i<arrays.length; i++){
				if(arrays[i].checked){
					...
					file_type = arrays[i].getAttribute("FILE_TYPE");
					num++;
				}
			}
			if(num != "1"){
		...
		function _showFileData(af_id){
			var url = path + "sce/lockchild/GetAdoptionPersonInfo.action?Flag=EN&type=A&AF_ID=" + af_id;
			_open(url,"window",950,600);
		}
		
	</script>
	<BZ:body property="searchData" codeNames="WJLX;SDFS;ADOPTER_CHILDREN_SEX;">
		<BZ:form name="srcForm" method="post" action="sce/lockchild/FileInfoSelect.action?type=A">
		<!-- 用于保存数据结果提示 -->
					...
					<table class="table table-striped table-bordered table-hover dataTable" adsorb="both" init="true" id="sample-table">
						<thead>
							<tr>
								...
								<th style="width: 10%;">
									<div class="sorting" id="REG_STATE">文件状态<br>File status</div>
								</th>
								<th style="width: 10%;">
									<div class="sorting" id="MATCH_STATE">操作<br>Operation</div>
								</th>
							</tr>
						</thead>
						<tbody>
						<BZ:for property="List">
							<tr class="emptyData">
								<td class="center">
									<input name="xuanze" type="radio" value="<BZ:data field="AF_ID" onlyValue="true"/>" FILE_TYPE='<BZ:data field="FILE_TYPE" defaultValue="" onlyValue="true"/>' MALE_NAME='<BZ:data field="MALE_NAME" defaultValue="" onlyValue="true"/>' FEMALE_NAME='<BZ:data field="FEMALE_NAME" defaultValue="" onlyValue="true"/>' class="ace">
								</td>
								<td class="center">
									<BZ:i/>
								</td>
								...
								<td class="center"><BZ:data field="REG_STATE" checkValue="0=unsubmitted;1=to be registered;2=to be supplemented;3=registered;" defaultValue="" onlyValue="true"/></td>
								<td class="center">
