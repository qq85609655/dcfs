<%
/**   
 * @Title: additionalAZB_list.jsp
 * @Description: 安置部预批补充查询列表
 * @author panfeng
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"sce/additional/findAddList.action?type=AZB&page=1";
		...
		function _addShow(){
			...
			if(num != "1"){
				...
				//var url = path + "sce/additional/additionalShow.action?showuuid=" + showuuid;
				//_open(url, this, 1000, 600);
				//window.open(path+"sce/additional/additionalShow.action?showuuid=" + showuuid + "&ra_id=" + ra_id,"newwindow","height=600,width=1000,top=70,left=180,scrollbars=yes");
				//document.srcForm.action = path+"sce/additional/additionalShow.action?showuuid=" + showuuid + "&ra_id=" + ra_id;
				//document.srcForm.submit();
	...
	<BZ:body property="data" codeNames="PROVINCE;ETXB;">
		<BZ:form name="srcForm" method="post" action="sce/additional/findAddList.action?type=AZB">
