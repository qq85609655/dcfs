...
<BZ:html>
	<BZ:head>
		...
		<script>
			...
			function _setSuppleShow(obj){
				...
				if(null!=val&&""!=val){
					$.ajax({
						url: path+'AjaxExecute?className=com.dcfs.ffs.audit.OpinionTemAjax&method=getAuditModelContent&TYPE=11&AUDIT_OPTION='+val+'&GY_TYPE='+gy_type,
						type: 'POST',
						timeout:1000,
						dataType: 'json',
			...
			function _goback(){
				var flag = "<%=flag %>";
				if(flag == "bc"){
					window.location.href=path+"sce/additional/findAddList.action?type=AZB";
				}else if(flag == "bf"){
					window.location.href=path+"sce/addTranslation/addTranslationList.action?type=AZB";
				}else{
			...
			function _submit(){
				...
				if (!runFormVerify(document.srcForm, false)) {
					...
					if(result == "4"){
						var last_state = $("#R_LAST_STATE2").val();
						if(last_state == "" || last_state == "null" || last_state == "2"){
							//表单提交
							document.srcForm.action = path+"sce/preapproveaudit/PreApproveAuditSave.action?type=AZB&Flag=<%=flag %>";
						...
						var atranslation_state = $("#R_ATRANSLATION_STATE2").val();
	...
	<BZ:body property="data" codeNames="SDFS;">
		...
		<BZ:input type="hidden" prefix="P_" field="ADD_TYPE" id="P_ADD_TYPE" defaultValue="2"/>
		...
		<BZ:input type="hidden" prefix="R_" field="LAST_STATE2" id="R_LAST_STATE2" defaultValue=""/>
		<BZ:input type="hidden" prefix="R_" field="ATRANSLATION_STATE2" id="R_ATRANSLATION_STATE2" defaultValue=""/>
					...
					<table class="bz-edit-data-table" border="0">
						...
						<tr>
							...
							<td class="bz-edit-data-value">
								<a href="#" target="_black" onclick="_showPreFile('<BZ:dataValue field="ORIGINAL_FILE_NO" defaultValue=""  onlyValue="true" />')" style="color: blue;">
							...
							<td class="bz-edit-data-value">
								<a href="#" target="_black" onclick="_showPreApprove('<BZ:dataValue field="PRE_REQ_NO" defaultValue="" onlyValue="true"/>')" style="color: blue;">
		...
		<div class="widget-box no-margin" style=" width:100%; margin: 0 auto">
			<iframe id="iframe" scrolling="no" src="<%=path %>/sce/preapproveapply/PlanOpinionShow.action?RI_ID=<%=ri_id %>&Flag=infoCN&type=show" style="border:none; width:100%; overflow:hidden;  frameborder="0"></iframe>
		</div>
		<!-- iframe显示区域end -->
					...
					<table class="bz-edit-data-table" border="0">
						<tr>
							...
							<td class="bz-edit-data-value" width="19%">
								<BZ:select prefix="R_" field="AUDIT_OPTION" id="R_AUDIT_OPTION" defaultValue="" formTitle="" notnull="审核结果不能为空！" onchange="_setSuppleShow(this)">
						...
						<tr class="SuppleInfo" style="display: none">
							<td class="bz-edit-data-title">允许修改抚育计划<br>和组织意见</td>
