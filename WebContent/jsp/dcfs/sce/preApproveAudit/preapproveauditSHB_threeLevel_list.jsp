...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditListSHB.action?Level=three&page=1";
		...
		function _audit(){
				...
				if(arrays[i].checked){
					...
					if(ri_state == "1" || ri_state == "2"){
						if(aud_state == "3"){
							...
							page.alert('请选择一条审核部状态为分管主任待审核的数据！');
							return;
						}
					}else{
			...
			if(num != "1"){
				...
				var rau_id = getStr('com.dcfs.sce.preApproveAudit.PreApproveAuditAjax','AUDIT_TYPE=1&AUDIT_LEVEL=2&RI_ID=' + ri_id);
				document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditShow.action?type=SHBadd&Level=2&RI_ID=" + ri_id + "&RAU_ID=" + rau_id + "&SPECIAL_FOCUS=" + is_focus;
				document.srcForm.submit();
			}
		}
		...
		function _show(type){
			...
			if(num != "1"){
				...
				document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditShow.action?Level=three&RI_ID=" + ri_id + "&type=" + type;
				document.srcForm.submit();
			}
		}
	...
	<BZ:body property="data" codeNames="PROVINCE;">
		<BZ:form name="srcForm" method="post" action="sce/preapproveaudit/PreApproveAuditListSHB.action?Level=three">
		<!-- 用于保存数据结果提示 -->
					...
					<table border="0" cellpadding="0" cellspacing="0" class="operation">
						<tr>
							<td><BZ:page
								...
								exportXls="true" 
								exportTitle="预批申请记录"
								exportCode="PROVINCE_ID=CODE,PROVINCE;SEX=FLAG,1:男&2:女&3:两性;BIRTHDAY=DATE;SPECIAL_FOCUS=FLAG,0:否&1:是;REQ_DATE=DATE;AUD_STATE2=FLAG,0:待审核&2:审核通过&3:审核不通过&1:审核中;AUD_STATE1=FLAG,0:经办人待审核&1:经办人审核中&2:部门主任待审核&3:分管主任待审批&4:审核不通过&5:审核通过&9:退回重审;LAST_STATE=FLAG,0:待补充&1:补充中&2:已补充;ATRANSLATION_STATE=FLAG,0:待翻译&1:翻译中&2:已翻译;RI_STATE=FLAG,1:待审核&2:审核中&3:审核不通过&4:审核通过&5:未启动&6:已启动&7:已匹配&9:无效;PASS_DATE=DATE;"
