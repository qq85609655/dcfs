...
<%
/**   
 * @Title: preapproveauditAZB_list.jsp
 * @Description: 预批申请审核列表 
 * @author yangrt
 * @date 2014-9-11
 * @version V1.0   
 */
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditListAZB.action?page=1";
		...
		function _reset(){
			...
			document.getElementById("S_ATRANSLATION_STATE2").value = "";
		...
		function _show(type){
			...
			if(num != "1"){
				...
				if(type == "AZBadd"){
					var rau_id = getStr('com.dcfs.sce.preApproveAudit.PreApproveAuditAjax','AUDIT_TYPE=2&RI_ID=' + ri_id);
					document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditShow.action?RI_ID=" + ri_id + "&RAU_ID=" + rau_id + "&type=" + type + "&SPECIAL_FOCUS=" + is_focus;
				}else if(type == "AZBshow"){
					document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditShow.action?RI_ID=" + ri_id + "&type=" + type;
				}
				document.srcForm.submit();
			}
		}
		...
		function _cancelApply(){
			...
			for(var i=0; i<arrays.length; i++){
				if(arrays[i].checked){
					var ri_state = arrays[i].getAttribute("RI_STATE");
					if(ri_state == "1" || ri_state == "2" || ri_state == "4" || ri_state == "5" || ri_state == "6"){
						ri_id = arrays[i].value;
						num++;
					}else{
						page.alert('请选择一条预批状态为已递交或审核中或审核通过或未启动或已启动的数据！');
						return;
					}
				}
			}
			if(num != "1"){
				...
				document.srcForm.action=path+"sce/preapproveaudit/PreApproveCancelApplyAdd.action?RI_ID=" + ri_id;
				document.srcForm.submit();
			}
		}
	...
	<BZ:body property="data" codeNames="PROVINCE;">
		<BZ:form name="srcForm" method="post" action="sce/preapproveaudit/PreApproveAuditListAZB.action">
		<!-- 用于保存数据结果提示 -->
							...
							<tr>
								...
								<td class="bz-search-value">
									<BZ:select prefix="S_" field="AUD_STATE1" id="S_AUD_STATE1" formTitle="" defaultValue="">
										...
										<BZ:option value="9">退回经办人</BZ:option>
									</BZ:select>
								</td>
								
								<td class="bz-search-title">补充状态</td>
								<td class="bz-search-value">
									<BZ:select prefix="S_" field="LAST_STATE2" id="S_LAST_STATE2" formTitle="" defaultValue="">
										<BZ:option value="">--请选择--</BZ:option>
								...
								<td class="bz-search-value">
									<BZ:select prefix="S_" field="ATRANSLATION_STATE2" id="S_ATRANSLATION_STATE2" formTitle="" defaultValue="">
										<BZ:option value="">--请选择--</BZ:option>
		...
		<div class="page-content">
			<div class="wrapper">
				...
				<div class="table-row table-btns" style="text-align: left">
					...
					<input type="button" value="审&nbsp;&nbsp;核" class="btn btn-sm btn-primary" onclick="_show('AZBadd')"/>&nbsp;
					<input type="button" value="撤销申请" class="btn btn-sm btn-primary" onclick="_cancelApply()"/>&nbsp;
					<input type="button" value="查&nbsp;&nbsp;看" class="btn btn-sm btn-primary" onclick="_show('AZBshow')"/>&nbsp;
					...
					<table class="table table-striped table-bordered table-hover dataTable" adsorb="both" init="true" id="sample-table">
						<thead>
							<tr>
								...
								<th style="width: 6%;">
									<div class="sorting" id="LAST_STATE2">补充状态</div>
								</th>
								<th style="width: 6%;">
									<div class="sorting" id="ATRANSLATION_STATE2">补翻状态</div>
								</th>
								<th style="width: 6%;">
							...
							<tr class="emptyData">
								...
								%>
									<input name="xuanze" type="radio" 
										...
										LAST_STATE2="<BZ:data field="LAST_STATE2" defaultValue="" onlyValue="true"/>" 
								...
								<td><BZ:data field="LAST_STATE2" defaultValue="" onlyValue="true" checkValue="0=待补充;1=补充中;2=已补充;"/></td>
								<td><BZ:data field="ATRANSLATION_STATE2" defaultValue="" onlyValue="true" checkValue="0=待翻译;1=翻译中;2=已翻译;"/></td>
					...
					<table border="0" cellpadding="0" cellspacing="0" class="operation">
						<tr>
							<td><BZ:page
								...
								exportXls="true" 
								exportTitle="预批申请记录"
								exportCode="PROVINCE_ID=CODE,PROVINCE;SEX=FLAG,1:男&2:女&3:两性;BIRTHDAY=DATE;SPECIAL_FOCUS=FLAG,0:否&1:是;REQ_DATE=DATE;AUD_STATE2=FLAG,0=待审核&2:审核通过&3:审核不通过;&4:审核中AUD_STATE1=FLAG,0:经办人待审核&1:经办人审核中&2:部门主任待审核&3:分管主任待审批&4:审核不通过&5:审核通过&9:退回经办人;LAST_STATE2=FLAG,0:待补充&1:补充中&2:已补充;ATRANSLATION_STATE2=FLAG,0:待翻译&1:翻译中&2:已翻译;RI_STATE=FLAG,1:待审核&2:审核中&3:审核不通过&4:审核通过&5:未启动&6:已启动&7:已匹配&9:无效;PASS_DATE=DATE;"
								exportField="ADOPT_ORG_NAME_CN=收养组织,15,20;MALE_NAME=男收养人,15;FEMALE_NAME=女收养人,15;PROVINCE_ID=省份,15;WELFARE_NAME_CN=福利院,15;NAME=儿童姓名,15;SEX=性别,15;BIRTHDAY=出生日期,15;SPECIAL_FOCUS=特别关注,15;REQ_DATE=申请日期,15;AUD_STATE2=安置部状态,15;AUD_STATE1=审核部状态,15;LAST_STATE2=补充状态,15;ATRANSLATION_STATE2=补翻状态,15;RI_STATE=预批状态,15;PASS_DATE=预批日期,15;"/>
							</td>
						</tr>
					</table>
				</div>
