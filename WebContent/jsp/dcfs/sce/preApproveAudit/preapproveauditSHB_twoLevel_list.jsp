...
<%
/**   
 * @Title: preapproveauditSHB_twoLevel_list.jsp
 * @Description: 审核部预批申请复审列表 
 * @author yangrt
 * @date 2014-9-11
 * @version V1.0   
 */
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditListSHB.action?Level=two&page=1";
		...
		function _audit(){
				...
				if(arrays[i].checked){
					...
					if(ri_state == "1" || ri_state == "2"){
						if(aud_state == "2"){
							...
							page.alert('请选择一条审核部状态为部门主任待审核的数据！');
							return;
						}
					}else{
			...
			if(num != "1"){
				...
				var rau_id = getStr('com.dcfs.sce.preApproveAudit.PreApproveAuditAjax','AUDIT_TYPE=1&AUDIT_LEVEL=1&RI_ID=' + ri_id);
				document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditShow.action?type=SHBadd&Level=1&RI_ID=" + ri_id + "&RAU_ID=" + rau_id + "&SPECIAL_FOCUS=" + is_focus;
				document.srcForm.submit();
			}
		}
		...
		function _show(type){
			...
			if(num != "1"){
				...
				document.srcForm.action=path+"sce/preapproveaudit/PreApproveAuditShow.action?Level=two&RI_ID=" + ri_id + "&type=" + type;
				document.srcForm.submit();
			}
		}
	...
	<BZ:body property="data" codeNames="PROVINCE;">
		<BZ:form name="srcForm" method="post" action="sce/preapproveaudit/PreApproveAuditListSHB.action?Level=two">
		<!-- 用于保存数据结果提示 -->
