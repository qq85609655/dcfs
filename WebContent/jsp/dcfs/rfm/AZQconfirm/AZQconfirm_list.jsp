...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"rfm/AZQconfirm/AZQconfirmList.action?page=1";
		...
		function _confirm(){
			...
			if(num != "1"){
				...
				document.srcForm.action=path+"rfm/AZQconfirm/confirmShow.action?showuuid="+showuuid;
				document.srcForm.submit();
			}
		}
		...
		function _show(){
			...
			if(num != "1"){
				...
				window.open(path + "rfm/AZQconfirm/showReturnFile.action?showuuid=" + showuuid,"newwindow","height=460,width=1000,top=70,left=180,scrollbars=yes");
			}
		}
		...
		function _cancel(){
			...
			if(num < 1){
				...
				if (confirm("确定取消选择的退文信息？")){
					...
					document.srcForm.action=path+"rfm/AZQconfirm/returnFileDelete.action";
					document.srcForm.submit();
				}
			}
		}
	...
	<BZ:body property="data" codeNames="WJLX;GJSY;SYS_GJSY_CN;SYZZ;TWCZFS_ALL;WJWZ">
		<BZ:form name="srcForm" method="post" action="rfm/AZQconfirm/AZQconfirmList.action">
		<!-- 用于保存数据结果提示 -->
