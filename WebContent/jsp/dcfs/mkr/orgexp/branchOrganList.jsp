	...
	<script>
		...
		function _modifyBranch(n){
			if(n.checked){
				...
				document.getElementById("iframeC").src = "<%=path %>/mkr/orgexpmgr/branchModify.action?ADOPT_ORG_ID=<%=id %>&ID="+n.value;
			}
		}
		
		function _create(){
			document.getElementById("iframeWin").style.display = "block";
			document.getElementById("iframeC").src = "<%=path %>/mkr/orgexpmgr/branchModify.action?ADOPT_ORG_ID=<%=id %>";
		}
		...
		function _delete(){
			...
			if(id != ""){
				if(confirm("Are you sure you want to delete? ")){
					document.srcForm.action = path+"mkr/orgexpmgr/deleteBranch.action?ADOPT_ORG_ID=<%=id %>&ID="+id;
					document.srcForm.submit();
					document.srcForm.action = path+"mkr/orgexpmgr/branchOrganList.action";
				}
			}else{
				alert("Please select a data you want to delete!");				
			}
		}
	</script>
	<BZ:body>
		<BZ:form name="srcForm" method="post" action="mkr/orgexpmgr/branchOrganList.action">
					...
					<table class="table table-striped table-bordered table-hover dataTable" style="word-break:break-all; overflow:auto;" adsorb="both" init="true" id="sample-table">
						<thead>
							<tr>
								...
								<th style="width: 20%;">
									<div class="sorting" id="AREA">所在地区(Region)</div>
								</th>
								<th style="width: 12%;">
									<div class="sorting" id="HEADER">负责人(Director)</div>
								</th>
								<th style="width: 12%;">
									<div class="sorting" id="TELEPHONE">电话(Telephone)</div>
								</th>
								<th style="width: 12%;">
									<div class="sorting" id="EMAIL">邮箱(Email)</div>
								</th>
								<th style="width: 12%;">
									<div class="sorting" id="FAX">传真(Fax)</div>
								</th>
								<th style="width: 20%;">
									<div class="sorting" id="ADDR">地址(Address)</div>
								</th>
							</tr>
						</thead>
						<tbody>
		...
		<div id="iframeWin" style="width:100%; margin: 0 auto;display: none;">
		 <iframe id="iframeC" scrolling="no" src="<%=path %>/mkr/orgexpmgr/branchModify.action?ADOPT_ORG_ID=<%=id %>" style="border:none; width:100%; height:400px; overflow:hidden; frameborder="0"></iframe>
		</div>
		</BZ:frameDiv>
		</BZ:form>
	</BZ:body>
