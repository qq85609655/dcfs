...
<BZ:html>
	...
	<script>
		...
		function _modifyBranch(n){
			if(n.checked){
				...
				document.getElementById("iframeC").src = "<%=path %>/mkr/orgexpmgr/branchModify.action?ADOPT_ORG_ID=<%=id %>&ID="+n.value+"&type=shb";
			}
		}
		
		function _create(){
			document.getElementById("iframeWin").style.display = "block";
			document.getElementById("iframeC").src = "<%=path %>/mkr/orgexpmgr/branchModify.action?ADOPT_ORG_ID=<%=id %>&type=shb";
		}
		...
		function _delete(){
			...
			if(id != ""){
				if(confirm("确认要删除选中信息吗? ")){
					...
					document.srcForm.submit();
					document.srcForm.action = path+"mkr/orgexpmgr/branchOrganList.action?type=shb";
				}
			}else{
				alert("请选择需要删除的数据！");				
			}
	...
	<BZ:body>
		...
		<div id="iframeWin" style="width:100%; margin: 0 auto;display: none;">
		 <iframe id="iframeC" scrolling="no" src="<%=path %>/mkr/orgexpmgr/branchModify.action?ADOPT_ORG_ID=<%=id %>&type=shb" style="border:none; width:100%; height:400px; overflow:hidden; frameborder="0"></iframe>
		</div>
		</BZ:frameDiv>
		</BZ:form>
	</BZ:body>
