...
<%@page import="hx.database.databean.DataList"%><BZ:html>
	...
	<script>
		...
		function _modif(){
			...
			if(num != 1){
				...
				document.srcForm.action=path+"mkr/organSupp/toWelfareModif.action?ID="+id+"&pro_code=<%=ID%>&type=modif";
			    document.srcForm.submit();
			}
			    document.srcForm.action=path+"mkr/organSupp/findWelfareList.action";
		}
		//跳转到查看页面
		function _detail(){
			var id="";
	...
	<BZ:body codeNames="PROVINCE;">
		<BZ:form name="srcForm"  method="post" action="mkr/organSupp/findWelfareList.action">
