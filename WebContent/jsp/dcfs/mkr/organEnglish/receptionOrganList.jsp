	...
	<script>
		$(document).ready(function() {
			...
			if(receptionId != "" && receptionId != "null"){
				iframeC.location = "<%=request.getContextPath() %>/mkr/organSupp/receptionModifyEn.action?receptionId="+receptionId+"&ID="+id;
			}
		});
		//添加在华旅行接待信息
		function _create(){
			...
			var id = "<%=id%>";
			iframeC.location = "<%=request.getContextPath() %>/mkr/organSupp/receptionModifyEn.action?ID="+id;
		}
		...
		function showReception(){
			...
			iframeC.location = "<%=request.getContextPath() %>/mkr/organSupp/receptionModifyEn.action?receptionId="+id;
		}
		...
		function _delete(){
			...
			if(num != 1){
				...
				document.srcForm.action = path+"mkr/organSupp/deleteReceptionEn.action?receptionId="+id;
				document.srcForm.submit();
			}
			document.location= "<%=request.getContextPath() %>/mkr/organSupp/receptionOrganListEn.action?ID=<%=id%>";
		}
	</script>
	<BZ:body property="data" codeNames="JDFLX;FZRLB;">
		<BZ:form name="srcForm" method="post" action="mkr/organSupp/receptionOrganListEn.action">
