<%
/**   
 * @Title: matchAuditAgain_list.jsp
 * @Description: 匹配复核列表
 * @author xugy
 * @date 2014-9-9下午7:15:31
 * @version V1.0   
 */
%>
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"matchAudit/matchAuditAgainList.action?page=1";
		...
		function _reset(){
			...
			document.getElementById("S_MATCH_STATE").value = "1";
		...
		function _auditAdd(){
			...
			if(num != "1"){
				page.alert('请选择一条部门主任待审核的数据');
				return;
			}else{
				document.srcForm.action=path+"matchAudit/toMatchAuditAgainAdd.action?ids="+ids;
				document.srcForm.submit();
			}
		}
		//
		function _detail(){
			...
			if(num != "1"){
				...
				document.srcForm.action=path+"matchAudit/matchAuditAgainDetail.action?ids="+ids;
				document.srcForm.submit();
			}
		}
	</script>
	<BZ:body property="data" codeNames="WJLX;PROVINCE;ETXB;CHILD_TYPE;GJSY;SYS_GJSY_CN;">
		<BZ:form name="srcForm" method="post" action="matchAudit/matchAuditAgainList.action">
					...
					<td style="width: 100%;">
						<table>
							...
							<tr>
								...
								<td>
									<BZ:select prefix="S_" field="MATCH_STATE" id="S_MATCH_STATE" width="148px" formTitle="匹配状态" defaultValue="1">
			...
			<div class="wrapper">
				...
				<div class="footer-frame">
					<table border="0" cellpadding="0" cellspacing="0" class="operation">
						<tr>
							<td><BZ:page form="srcForm" property="List" exportXls="true" exportTitle="部门主任审核数据" exportCode="FILE_TYPE=CODE,WJLX;PROVINCE_ID=CODE,PROVINCE;SEX=CODE,ETXB;CHILD_TYPE=CODE,CHILD_TYPE;OPERATION_STATE=FLAG,0:待处理&1:处理中&2:已处理;MATCH_STATE=FLAG,0:经办人待审核&1:部门主任待审核&3:审核通过&4:审核不通过&9:无效;REGISTER_DATE=DATE;BIRTHDAY=DATE,yyyy/MM/dd" exportField="FILE_NO=收文编号,15,20;REGISTER_DATE=收文日期,15;COUNTRY_CN=国家,15;NAME_CN=收养组织,20;MALE_NAME=男方,15;FEMALE_NAME=女方,15;FILE_TYPE=文件类型,10;MATCH_NUM=匹配次数,10;CHILD_TYPE=儿童类型,10;PROVINCE_ID=省份,10;WELFARE_NAME_CN=福利院,20;NAME=姓名,15;SEX=性别,10;BIRTHDAY=出生日期,15;MATCH_STATE=匹配状态,20;"/></td>
						</tr>
					</table>
				</div>
				<!--分页功能区End -->
