...
<BZ:head>
	...
	function _submit(){
		if (confirm('确定提交匹配结果？')) {
			document.srcForm.action=path+"mormalMatch/saveMatchResult.action?AFid=<%=AFid%>&CIid=<%=CIid%>";
	...
	function _goback(){
		if (confirm('匹配结果未保存，确定返回儿童列表？')) {
			document.srcForm.action=path+"mormalMatch/normalMatchCIList.action?AFid=<%=AFid%>";
			document.srcForm.submit();
		}
	}
	//关闭弹出页面
