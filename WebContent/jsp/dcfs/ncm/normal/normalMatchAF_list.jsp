<%
/**   
 * @Title: normalMatchAF_list.jsp
 * @Description: 匹配列表--收养人列表
 * @author xugy
 * @date 2014-9-2下午4:59:31
 * @version V1.0   
 */
%>
...
<BZ:html>
	...
	<script>
		...
		//执行按条件查询操作
		function _search(){
			document.srcForm.action=path+"mormalMatch/normalMatchAFList.action?page=1";
		...
		//正常儿童预分配计划统计
		function _matchPlan(){
			document.srcForm.action=path+"mormalMatch/toMatchPlanCount.action";
			document.srcForm.submit();
		}
		//正常儿童匹配
		function _match(){
			...
			if(num != "1"){
				...
				$.layer({
					...
					shade : [0.5 , '#D9D9D9' , true],
					//page : {dom : '#planList'},
					iframe: {src: '<BZ:url/>/mormalMatch/normalMatchCIList.action?AFid='+AFid}
				}); */
				
				window.open(path + "mormalMatch/normalMatchCIList.action?AFid="+AFid,"",",'height=800,width=1200,top=100,left=100,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no'");
			}
		}
		//保存匹配结果后，刷新列表
		function _matchResultView(result){
			//layer.closeAll();
			document.srcForm.action=path+"mormalMatch/normalMatchAFList.action?page=1&result="+result;
			document.srcForm.submit();
		}
	</script>
	<BZ:body property="data" codeNames="WJLX;WJLX_DL;GJSY;SYS_GJSY_CN;">
		<BZ:form name="srcForm" method="post" action="mormalMatch/normalMatchAFList.action">
					...
					<td style="width: 100%;">
						<table>
							...
							<tr>	
								...
								<td>
									<BZ:select prefix="S_" field="FILE_TYPE" id="S_FILE_TYPE" isCode="true" codeName="WJLX_DL" width="148px" formTitle="文件类型" defaultValue="">
										<BZ:option value="">--请选择--</BZ:option>
									</BZ:select>
								</td>
		...
		<div class="page-content">
			<div class="wrapper">
				...
				<div class="table-row table-btns" style="text-align: left">
					...
					<input type="button" value="匹配计划" class="btn btn-sm btn-primary" onclick="_matchPlan()"/>&nbsp;
					...
					<table class="table table-striped table-bordered table-hover dataTable" adsorb="both" init="true" id="sample-table">
						<thead>
							<tr>
								...
								<th style="width: 6%;">
									<div class="sorting" id="EXPIRE_DATE">到期期限</div>
								</th>
								<th style="width: 6%;">
