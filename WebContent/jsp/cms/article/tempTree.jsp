...
<BZ:head>
	...
	function L(id,selNode){
		...
		//全部则显示所有待审记录列表
		<%-- if(parentId == 0){
			//parent.mainFrame.location = "<%=path %>/article/Article!tempQuery.action?<%=Article.CHANNEL_ID%>=0";
			childFrame.location = "<%=path %>/article/Article!tempQuery.action?<%=Article.CHANNEL_ID%>=0";
		}else{
			//处理
			//parent.mainFrame.location = "<%=path %>/article/Article!tempQuery.action?<%=Article.CHANNEL_ID%>="+id;
			childFrame.location = "<%=path %>/article/Article!tempQuery.action?<%=Article.CHANNEL_ID%>="+id;
		} --%>
		childFrame.location = "<%=path %>/article/Article!tempQuery.action?<%=Article.CHANNEL_ID%>="+id;
	}
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0" align="center">
		<tr>
			...
			<td width="75%" valign="top">
				<iframe id="childFrame" name="childFrame" src="<BZ:url/>/article/Article!tempQuery.action?<%=Article.CHANNEL_ID%>=0" style="width: 100%;" frameborder="0" scrolling="no"></iframe>
			</td>
		</tr>
	</table>
</BZ:body>
