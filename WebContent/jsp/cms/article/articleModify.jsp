...
<script>
	function tijiao()
	{
		...
		document.srcForm.action=path+"article/Article!modifyDirect.action";
	 	document.srcForm.submit();
 	
 		//关闭，并刷新上级页面
 		window.close();
 		window.opener.document.srcForm.action =path+"article/Article!query.action?<%=Article.CHANNEL_ID%>=<%=(String)request.getAttribute(Article.CHANNEL_ID) %>";
		...
		<td height="100%">
			...
			<td>
				<up:uploadBody 
					...
					packageId="<%=data.getString(Article.BODY_TEXT_ATT) %>"
			...
			<td></td>
			<td>外部链接</td>
			<td><BZ:input field="OUT_LINK" prefix="Article_" defaultValue=""/></td>
			<td></td>
			<td></td>
			<td></td>
			</tr>
