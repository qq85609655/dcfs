 ...
 * Method:
 ************************************/
function SimpleGrid(tblId,formname){
	//公共属性
...
function sg_initTHead(){
	if(this.oTHead!=null&&this.oTHead.rows[0]!=null){
		...
		for(var i=0;i<oThs.length;i++){
			...
			if(sortType!="none"){
			  if(this.oTitles[i].childNodes[0].name=='jsp'){
				this.bindListener(this.oTitles[i].childNodes[0],"onclick",this.sortClick());
				}
			  else if(this.oTitles[i].childNodes[0].name=='database'){
			...
			if(this.oTitles[i].childNodes[0].id==compositor){
			if(ordertype=='asc'){
		...
		if(!sg.isToolClick){
			...
			if(oTitle.getAttribute("sortType")!="none"){
				...
				if(clickTd.id=='true'&&sg.lastItem==thisTool){
					...
					if(srcEle.parentElement.parentElement.childNodes[0].name=='database'){
					sg.bindListener(up_item,"onclick",sg.sort_database_Menu('asc',srcEle.parentElement.parentElement.childNodes[0].id));
					}else{
				...
				if(clickTd.id=='false'&&sg.lastItem==thisTool){
					...
					if(srcEle.parentElement.parentElement.childNodes[0].name=='database'){
					sg.bindListener(down_item,"onclick",sg.sort_database_Menu('desc',srcEle.parentElement.parentElement.childNodes[0].id));
					}else{
...
function sg_sort_database_Click(){
	...
	var srcEle=event.srcElement;
	var form=sg.form;
	document.getElementById('compositor').value=srcEle.parentElement.childNodes[0].id;
...
function sg_styleTrChange(trObj,isIn,i){
	...
	if(isIn){
		return function(){
            ...
            trObj.childNodes[0].childNodes[3].style.display='block';
            }
		...
		return function(){
            ...
            trObj.childNodes[0].childNodes[3].style.display='none';
            }
            }catch(e){
            }
...
function sg_styleTrClick(oTr){
	...
	return function(){
		...
		oTr.childNodes[0].childNodes[3].style.display='none';
		oTr.childNodes[0].childNodes[2].style.display='block';
		...
		oTr.childNodes[0].childNodes[2].style.display='none';
		oTr.childNodes[0].childNodes[3].style.display='block';
