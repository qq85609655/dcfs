...
t.dynamic  = true;
//复选框只选择自己
t.onlySelectSelf = false;
//只能从选择叶子节点
t.onlyCheckChild=false;
t.TreeObjName="";
t.useCheckbox = false;
t.autoFocused = false;
...
MzTreeView.prototype.epAll = function(id)
{
  if("undefined"==typeof id) return;
  var node=this.getNodeById(id);
  if(!node||node.hasChlid) return;
...
MzTreeNode.prototype.render = function(last)
{
  ...
  s += "<div id='oneNode_div_" + this.id + "'><table border='0' cellpadding='0' cellspacing='0'>"+
  ...
  s += "</td><td style='padding-left: 3px' nowrap='true'><a name='" + this.id + "' href='"+ url +
...
MzTreeNode.prototype.epAll = function()
{
	var v=0;
  if(this.hasChild && !this.expanded) this.expand();
  for(var x=this.$$caller.index, i=0; i<this.childNodes.length; i++)
  {
    var node = this.childNodes[i]; if (node.hasChild)
    v=v+Instance(x).nodes[node.index].epAll();
  }
  return v;
};
