...
var page={
	...
	handle_side_menu:function() {
		...
		$("#sidebar .nav-list").on(page.click_event, function(h) {
			...
			if (!g.hasClass("dropdown-toggle")) {
				...
				$("#breadcrumbs .active").remove();
				var title = '';
				g.parents(".submenu").each(function(){
					var prev = $(this).prev();
					title = '<li class="active"><span class="icon-raquo">&rsaquo;</span> '+$(prev).attr("title")+' </li>'+title;
				});
				$("#breadcrumbs ul").append(title);
				if(!g.hasClass("front-menu")) {
					$("#breadcrumbs ul").append('<li class="active"><span class="icon-raquo">&rsaquo;</span> '+g.attr("title")+' </li>');
				}
				page._goTop();
	...
	_runGoTop : function() {
		...
		if (scrollTop > 1) {
			var topElement = "<div class=\"bz-gotop\" onclick=\"page._goTop();\" title=\"回到顶部\"></div>";
