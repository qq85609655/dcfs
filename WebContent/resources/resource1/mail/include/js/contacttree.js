...
function _show_all_name(sel_node){
	var rValue = sel_node.T;
	var _n = sel_node.parentNode;
	if (_n){
		var tt = _n.T;
		var _pn = _n.parentNode;
		if(_pn!=null){
			if (_pn.id=="0" || tt==""){
				return rValue;
			}
		}
		return _show_all_name(_n) + "-" + rValue;
	}else{
		return rValue;
	}
}
function _sel(){
	...
	for(var i=0;i<es.length;i++)
	{
		if (es[i].checked) {
			...
			var one = new Array();
			var node = es[i];
			var selNode = _getSelctNode(node);
			//判断是否是要显示上级的名称
			var show_parent = document.getElementsByName("showParent");
			var showName ="";
			if (show_parent!=null && show_parent.length>0){
				...
				if (show_parent!=null){
					var showP = show_parent.value;
					if (showP.toUpperCase()=="TRUE"){
						showName=_show_all_name(selNode);
					}
				}
			}
			if (showName==""){
			...
			one["name"]=showName
			//one["name"]=node.getAttribute("desc");
			one["type"]=selNode.V; 
			one["value"]=es[i].value;
			reValue[reValue.length]=one;
...
{
  var a = new Array();
  for (var id in this.N){ 
  	a[a.length] = id;
  	var T  = this.getAttribute(this.N[id], "T");//增加检索数据 lizb
  	var sid   = id.substring(id.lastIndexOf("_")+1);//增加检索数据 lizb
  	sNode[sNode.length] = sid + "|" + T;//增加检索数据 lizb
  }
...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var S  = this.getAttribute(source, "S");
  var X  = this.getAttribute(source, "X");
  var V = this.getAttribute(source, "V");
  ...
  this.node[this.index] =
  {
    ...
    "S" : S,   //lizb modify on 2008-7-24 03:28
    "X" : X  //lizb modify on 2008-7-24 03:28
  };
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if( node.ctrl.length > 0 ) HTMCHK = "<input nodeId='" + id + "' type='checkbox' desc='" + node.T + "' name='"+ node.ctrl +"' value='"+ sid +
  	"' id='"+ this.name +"_checbox_"+ id +"' style='height:15px;'"+
  	" onclick=\""+ this.name +".checkChild('"+ id +"'," + node.X + "," + node.S + ");"
  	+this.name+".expandSelf(this,"+id+")"+"\" "
  	+ HTMCHKED +">";				//Rexliu add on 2007-1-23 14:29
  ...
  try {
	...
	var o_click=" onclick=\"return "+ this.name +".nodeClick('"+ id +"');\">";
  HTML += "<IMG "+
    "align='absMiddle' "+
...
//点击展开树节点的对应方法
MzTreeView.prototype.expand1   = function(id, sureExpand)
{
  ...
  if (area)
  {
    ...
    if(!node.isLoad)
    {
      ...
      if(node.hasChild && node.childNodes.length>100)
      {
...
MzTreeView.prototype.expandSelfBody =function(id)
{
     var n=this.node[id];
     n.isExpand=false;
     if (n.ctrl.length>0){
     if(n.hasChild){
	     ...
	     {
	       // this.expandSelf(null,n.childNodes[k].id);
	        tempnode = n.childNodes[k];
	        //tempnode.isExpand=false;
	         checkNode =document.getElementById(this.name +"_checbox_"+ n.childNodes[k].id);
	       // checkNode.click();
	       checkNode.checked=true;
	        this.expandSelfBody(tempnode.id);
	      // this.expand(tempnode.id);
	     }
     }else{
    		checkNode = document.getElementById(this.name +"_checbox_"+ n.id);
    		//checkNode.click();
    		checkNode.checked=true;
 	 }
 	 }
}
MzTreeView.prototype.collapseSelfBody =function(id)
{
     var n=this.node[id];
     if (n.ctrl.length>0){
     if(n.hasChild){
     	 ...
     	 for (var k=0; k<n.childNodes.length; k++)
	     {
	       // this.expandSelf(null,n.childNodes[k].id);
	        tempnode = n.childNodes[k];
	       // tempnode.isExpand=true;
	        this.collapse(tempnode.id);
	        checkNode =document.getElementById(this.name +"_checbox_"+ n.childNodes[k].id);
	     //   checkNode.click();
	     checkNode.checked=false;
	     this.collapseSelfBody(tempnode.id);
	     }
     }else{
    		checkNode = document.getElementById(this.name +"_checbox_"+ n.id);
    		//checkNode.click();
...
MzTreeView.prototype.expandSelf =function(obj,id)
{
   
   var node=this.node[id];
   if(obj.checked)
   { 
      this.expandSelfBody(id);
   }
   else
   {
    //  this.collapseSelfBody(id);
       if(node.hasChild){
     	//this.collapse(id);
     	 for (var k=0; k<node.childNodes.length; k++)
	     {
	        tempnode = node.childNodes[k];
	       // this.collapse(tempnode.id);
	        checkNode =document.getElementById(this.name +"_checbox_"+ node.childNodes[k].id);
	        checkNode.checked=false;
	        this.collapseSelfBody(tempnode.id);
	     }
	     }
...
//树的单击事件处理函数，如果是复选，那么单击就打开/关闭有子节点的节点，如果是叶子节点，那么就直接选中
MzTreeView.prototype.clickHandle = function(e)
{
  ...
  switch(e.tagName)
  {
    ...
    case "A" :
      if(e.id) {
      var idIndex = e.id.substr(e.id.lastIndexOf("_") + 1);
      //lizb 扩展单击展开
      var n=this.node[idIndex];
      if (n.ctrl.length>0){
	      if(n.hasChild){
	      	this.expand(idIndex);
	      }else{
...
//MzTreeView 双击事件的处理函数,双击名称，如果是复选框，并且包含子节点，那么就展开,单选时双击叶子节点直接选中
...
MzTreeView.prototype.collapse1 = function(id)
{ 
  var node  = this.node[id];
  if (!node.hasChild) return;
...
//展开root的所有节点
MzTreeView.prototype.expandRoot = function()
{
 if(!this.currentNode) this.currentNode=this.node["0"];
...
MzTreeView.prototype.focusClickNode      = function(id)
{
 try {
  ...
  if(!this.currentNode) this.currentNode=this.node["0"];
   var ckboxID = this.name +"_checbox_"+ id;
    ...
    if(root.hasChild)
    {
      ...
      for (var k=0; k<root.childNodes.length; k++)
      {
        if (root.childNodes[k].sourceIndex == sourceIndex)
        {
          ...
          var ckboxID = this.name +"_checbox_"+ root.id;
		  var ckbox = this.getElementById(ckboxID);
		  if(ckbox)
		  {
		    ckbox.checked=true;
...
MzTreeView.prototype.checkChild = function(id,child,parent)
{
	if(child)
	this.selChild(id);
	//if(parent)
...
function _search(o){
	_v = o.value;
	if (_v==""){
		_printMessage("请输入查询条件");
		return;
	}
	sN = new Array();
	sIndex =-1;
	sNode.sort(function(a,b){return a.indexOf(_v)==-1?1:-1;});
	var l = sNode.length;
	for(var i=0;i<l;i++){
		if (sNode[i].indexOf(_v)==-1){
			break;
		}else{
			sN[sN.length]=sNode[i];
		}
	}
	sLength = sN.length;
	if (sLength>0){
...
//点击焦点触发
function _clickIndex(index){
	var sv = sN[index].split("|");
	_printMsg(sv[1] + "(" + sv[0] + ")");
	tree.Click(sv[0]);
}
//提示消息
function _printMsg(s){
	if (sLength<1){
		searchMsg.innerHTML="<span class='search'>没有检索到数据，请更换条搜索条件重试!</span>";
		...
		//|<a href='#' class='MzTreeview' onclick='_previous()'>上一条</a>|<a href='#' class='MzTreeview' onclick='_next()'>下一条</a>|
		searchMsg.innerHTML="<span title='" + s + "' class='search'>共" + sLength + "条,第" + (sIndex+1) + "条:[<b><font color='#004040'>" + s1 + 
		"</font></b>]</span>";
		document.all("p").style.display="";
		document.all("n").style.display="";
	}
}
function _printMessage(s){
	searchMsg.innerHTML="<span class='search'>" + s + "</span>";
	document.all("p").style.display="none";
		document.all("n").style.display="none";
}
//按键搜索
