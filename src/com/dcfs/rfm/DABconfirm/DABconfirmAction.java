package com.dcfs.rfm.DABconfirm;
...
/**
 * 
 * @ClassName: DABconfirmAction 
 * @Description: 由档案部对退文信息进行查询、确认、查看、取消退文、导出操作
 * @author panfeng;
 * @date 2014-9-25 下午3:11:02 
 *
 */
public class DABconfirmAction extends BaseAction {
	
	private static Log log = UtilLog.getLog(DABconfirmAction.class);

    private DABconfirmHandler handler;
    ...
    public DABconfirmAction(){
        this.handler=new DABconfirmHandler();
    } 
	...
	public String DABconfirmList(){
		...
		Data data = getRequestEntityData("S_","MALE_NAME","FEMALE_NAME","FILE_NO","REGISTER_DATE_START",
					"REGISTER_DATE_END","COUNTRY_CODE","ADOPT_ORG_ID","APPLE_TYPE","PERSON_NAME",
					"RETREAT_DATE_START","RETREAT_DATE_END","RETURN_STATE");
		...
		try {
			...
			//5 获取数据DataList
			DataList dl=handler.DABconfirmList(conn,data,pageSize,page,compositor,ordertype);
			//6 将结果集写入页面接收变量
			...
			if (conn != null) {
				try {
					...
					if (log.isError()) {
						log.logError("DABconfirmAction的DABconfirmList.Connection因出现异常，未能关闭",e);
					}
					retValue = "error2";
				}
			}
	...
	public String DABconfirmSave(){
        ...
        try {
        	...
        	fileData.add("RETURN_STATE", "2");//退文状态为"待处置"(文件信息表)
        	data.add("RETURN_STATE","2");//退文状态为"待处置"（退文记录表）
            
            success=handler.DABconfirmSave(conn,data,fileData);
            if (success) {
            ...
            if (conn != null) {
                try {
                    ...
                    if (log.isError()) {
                        log.logError("DABconfirmAction的Connection因出现异常，未能关闭",e);
                    }
                    e.printStackTrace();
                }
            }
