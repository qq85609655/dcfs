package com.dcfs.rfm.BGSconfirm;
...
/**
 * 
 * @ClassName: BGSconfirmAction 
 * @Description: 由办公室对退文信息进行查询、确认、查看、取消退文、导出操作
 * @author panfeng;
 * @date 2014-9-23 下午7:12:22 
 *
 */
public class BGSconfirmAction extends BaseAction {
	
	private static Log log = UtilLog.getLog(BGSconfirmAction.class);

    private BGSconfirmHandler handler;
    ...
    public BGSconfirmAction(){
        this.handler=new BGSconfirmHandler();
    } 
	...
	public String BGSconfirmList(){
		...
		try {
			...
			//5 获取数据DataList
			DataList dl=handler.BGSconfirmList(conn,data,pageSize,page,compositor,ordertype);
			//6 将结果集写入页面接收变量
			...
			if (conn != null) {
				try {
					...
					if (log.isError()) {
						log.logError("BGSconfirmAction的BGSconfirmList.Connection因出现异常，未能关闭",e);
					}
					retValue = "error2";
				}
			}
	...
	public String BGSconfirmSave(){
        ...
        try {
            ...
            success=handler.BGSconfirmSave(conn,data,fileData);
            if (success) {
            ...
            if (conn != null) {
                try {
                    ...
                    if (log.isError()) {
                        log.logError("BGSconfirmAction的Connection因出现异常，未能关闭",e);
                    }
                    e.printStackTrace();
                }
            }
