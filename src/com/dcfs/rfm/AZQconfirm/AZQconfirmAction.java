...
/**
 * 
 * @ClassName: AZQconfirmAction 
 * @Description: 由翻译公司对退文信息进行查询、确认、查看、取消退文、导出操作
 * @author panfeng;
 * @date 2014-9-28 上午11:45:51 
 *
 */
public class AZQconfirmAction extends BaseAction {
	
	private static Log log = UtilLog.getLog(AZQconfirmAction.class);

    private AZQconfirmHandler handler;
    ...
    public AZQconfirmAction(){
        this.handler=new AZQconfirmHandler();
    } 
	...
	public String AZQconfirmList(){
		...
		try {
			...
			//5 获取数据DataList
			DataList dl=handler.AZQconfirmList(conn,data,pageSize,page,compositor,ordertype);
			//6 将结果集写入页面接收变量
			...
			if (conn != null) {
				try {
					...
					if (log.isError()) {
						log.logError("AZQconfirmAction的AZQconfirmList.Connection因出现异常，未能关闭",e);
					}
					retValue = "error2";
				}
			}
	...
	public String AZQconfirmSave(){
        ...
        try {
        	...
        	tranferData.add("TRANSFER_CODE", TransferCode.RFM_FILE_FYGS_DAB);
            ...
            success=handler.AZQconfirmSave(conn,data,fileData);
            if (success) {
            ...
            if (conn != null) {
                try {
                    ...
                    if (log.isError()) {
                        log.logError("AZQconfirmAction的Connection因出现异常，未能关闭",e);
                    }
                    e.printStackTrace();
                }
            }
