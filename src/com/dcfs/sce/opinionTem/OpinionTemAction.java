...
public class OpinionTemAction extends BaseAction{
	...
	/**
	 * @Title: SHBfindListTem 
	 * @Description: 审核部预批审核意见模板列表
	 * @author: panfeng
	 * @return String    返回类型 
	 * @throws
	 */
	public String SHBfindListTem(){
		// 1 设置分页参数
		...
		try {
			...
			conn = ConnectionManager.getConnection();
			//5 获取数据DataList
			DataList dl=handler.SHBfindListTem(conn,pageSize,page,compositor,ordertype);
			//6 将结果集写入页面接收变量
			...
			if (conn != null) {
				try {
					...
					if (log.isError()) {
						log.logError("OpinionTemAction的SHBfindListTem.Connection因出现异常，未能关闭",e);
					}
					retValue = "error2";
				}
			}
	...
	/**
	 * @Title: AZBfindListTem 
	 * @Description: 安置部预批审核意见模板列表
	 * @author: panfeng
	 * @return String    返回类型 
	 * @throws
	 */
	public String AZBfindListTem(){
		// 1 设置分页参数
		int pageSize = getPageSize(DcfsConstants.DEFAULT_PAGESIZE);
		int page = getNowPage();
		if (page == 0) {
		...
		try {
			...
			conn = ConnectionManager.getConnection();
			//5 获取数据DataList
			DataList dl=handler.AZBfindListTem(conn,pageSize,page,compositor,ordertype);
			//6 将结果集写入页面接收变量
			setAttribute("List",dl);
			setAttribute("compositor",compositor);
			setAttribute("ordertype",ordertype);
			...
			if (conn != null) {
				try {
					...
					if (log.isError()) {
						log.logError("OpinionTemAction的AZBfindListTem.Connection因出现异常，未能关闭",e);
					}
					retValue = "error2";
				}
			}
